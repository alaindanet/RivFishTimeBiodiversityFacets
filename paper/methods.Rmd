...
States (Table S1), following the same criteria than RivFishTime for integration[@comte_rivfishtime_2021].
...
complement of the Jaccard index (J) and the Simpson-based dissimilarity index[@hillebrand_biodiversity_2018] ($H_d$). The former index is based on
...
community, i.e. that a community is a subset of the other[@baselga_temporal_2015]. We took the first year of sampling of a community as
...
We computed $R^2$ to assess the quality of the fit of the bayesian models.
...
computed marginal ($R^2_m$)and conditional ($R^2_c$) R-squared, respectively
associated to the variance explained by fixed effects and the one explained by
both fixed and random effects [@nakagawa_general_2013]. We only included
