...
between raw species richness and chao richness ($\rho = `r cor_chao_rich`$ for
both raw variables and log transformed ones, Fig. SXX).
...
occurrences, immigration rates and community composition [ref].
To capture this stream gradient, we described stream characteristics at each site by the
...
so by snapping the sites to the closest stream segment using a one kilometer
...
minimum values plus one to avoid values $\leq 0$,
$x^{\prime}_i = x_i + min(x_i) + 1$) and standardizing all the variables, i.e. centering and scaling
. We orthogonally rotated the two first principal
components, using the varimax criterion [@kaiser_varimax_1958, @revelle_psych_2019],
to increase the quality of the variable representation (i.e. their loadings) on
the two first principal components. The first rotated component,
related to average annual discharge, distance from source and Strahler order
and capturing `r perc_var_rc1` of the variance (Fig. S\@ref(fig:pca-riv-str)),
was used as a composite variable describing the stream gradient from upstream to
downstream.
...
navigable pathways. It does so by combining remote sensing data,
...
of 1993 as a measure of the legacy of anthropogenic pressures and the ratio
...
both for species richness and abundances.
...
- A proposition:
  - Present the philosophy: response versus drivers: why not all interactions?
    justify them
...
quality of the model fitting to the data, decreasing the AIC by 29% in average
from 0% for total abundance to 124% for the Simpson based dissimilarity index
(Table S\@ref(tab:aic)) [missing chao richness and
...
unit of time [@van_klink_meta-analysis_2020].
...
effect [@van_klink_meta-analysis_2020; @mastrandrea_guidance_2010].
...
\dfrac{1}{\sqrt{\tau}}$). We checked that the 
slope coefficients, random effects and the temporal trends by basin and site
were the same than with a implementation in frequentist. Then, we concluded that
the quality of parameter inference did not suffer from the uninformative priors.

### Model validity

We checked the model validity visually by plotting the fitted versus the
observed values [ref plot].

- Add plots of posterior marginal distribution of the parameters? (it would be
  like 8*10 plots for the fixed effects only)
...
level. A PCA finds the linear combination of variables that explained the most
...
biodiversity [@schlaepfer_non-native_2018] and site temporal trends in percentage of
non-native species displayed very little variation, then it was of little
...
proportion of the most outlying data trimmed ($alpha$) [ref plot]. We thus
