...
completeness of the time series was of `r mcpl` (see Fig. SX-X for the complete distribution).
The sites were
...
`r dec_waic` in average (`r perc_dec_waic`) (Table S\@ref(tab:aic)). It
...
unit of time (See Table SXX for a summary of variable transformations).  
...
```{r}
tar_load(r2_obs_fit)
r2 <- c(min = min(r2_obs_fit$gelman_r2), max = max(r2_obs_fit$gelman_r2))
```

We checked the model validity visually by plotting the fitted versus the
observed values (Fig. SXX). We visually inspected the PIT and CPO (respectively
...
[$R^2 = Var_{fit} / (Var_{fit} + Var_{res}) = \sigma^2(\hat{y_i}) /
(\sigma^2(\hat{y_i}) + \sigma^2(y_i - \hat{y_i}))$ with $y_i$ and $\hat{y_i}$
being respectively the observations and the predicted values, $Var_{fit}$ and
Var_{res} being respectively the variance of predictive means and the variance
of the residuals, @gelman_r-squared_2019]. $R^2$ spans from `r r2$min` for
nestedness to `r r2$max` for species richness (See Fig. SXX), indicating that the
the predictors namely time, anthropogenic pressures, and stream
gradient as well as the hierarchical structure of the models all together
capture a significant share of the variance.
