...
heterogeneity is also apparent within the same river basin (Supplementary Data
S1). For example, on average the Thames basin show the same spatial patterns than at the
...
trajectories (Fig. 2c). The largest cluster was characterised by moderate
...
longitudinal stream position (i.e. represented by a synthetic PCA axis based on several hydromorphological characteristics where high values are associated with more downstream areas, see Methods), such as the legacy effects of past anthropogenic pressures on total
...
or coverage-based species richness (See Methods, Extended Data Fig. 7).
...
Jaccard and Simpson-based dissimilarity (Extended Data Fig. 7) but the effects of past
...
methods (e.g. electrofishing, seining, Supplementary Table S2), we selected for each site only
...
(`r per_pays["USA"]`, Supplementary Table S2).
...
non-native species and community composition (Supplementary Table S2). Total abundance was
...
`r u_per["CPUE"]`), and Leslie index (`r u_per["Leslie_index"]`, Supplementary Table S2).
...
Supplementary Table S2). For the sites falling outside the river basins provided in the global
...
Supplementary Table S2). We then estimated the percentage of non-native species with respect
to both abundance and species richness for each sampling event (Supplementary Table S3).
...
correlation of 0.09 found between past and recent pressures, Supplementary Fig. S1).
...
(`r perc_dec_waic`) across community metrics facets (Supplementary Table S4). It suggested
...
fitted versus observed values (Supplementary Fig. S2). We visually inspected the PIT and
...
as all Variance Inflation Factors were around 1 (Supplementary Table S5).
...
outlying data trimmed ($\alpha$) and the number of clusters (Supplementary Fig. S3). We
thus selected a partition of temporal community changes in six clusters with
$\alpha=5\%$ (see Supplementary Fig. S4 with four clusters). We did not constrain the algorithm for the
...
study (Supplementary Table S1). We further thank Suzanne
