...
characterized by significant changes of the dominant species, together with an
increase in total abundance of 13% per decade, and species richness of 7% per
decade. An important spatial heterogeneity in these trends could be traced back
to the strength and timing of anthropogenic pressures and was mediated by
...
consistent with previously reported biotic homogenization at global scale.
...
Non-native species can also play a disproportionate role in the recomposition of
...
resulting in the widespread homogenization of historically distinct
faunas[@olden_homogocene_2018]. Hubs of human activities such as human
settlements, transport and trade are also responsible for major habitat
alterations and increased accessibility, resulting in more frequent non-native
introduction events and opportunities for spread[@chapman_invasion_2020;
...
non-native species that can increase community total abundance and species
richness[@mcgill_fifteen_2015; @primack_biodiversity_2018]. Understanding
...
Moreover, habitat structure and connectivity can enhance or dampen community
responses to anthropogenic pressures by mediating dispersal among
habitats[@altermatt_river_2013].

Here, we investigated how, and why riverine fish communities change spatially
and temporally from local to continental extents. To do so, we leveraged a
compilation of 4,476 riverine fish community timeseries[@comte_rivfishtime_2021]
that had been repeatedly sampled over the last three decades in various river
basins across three biogeographic realms. We quantified temporal changes in
total abundance, species richness, and community composition across local
communities, including in the share of non-native species. We next characterised
the typology of community temporal trends by examining the covariation among
different communiy metrics, and distribution of community trajectories across
spatial scales. We finally assessed how fish community changes could be traced
back to the spatio-temporal changes in anthropogenic pressures and position
along the longitudinal stream gradient. This study aims to further our
understanding of the complexity of local community changes by addressing the
effects of anthropogenic pressures and advancing new knowledge necessary to
inform actions seeking to curb the current freshwater biodiversity crisis.
...
trends_cap <- paste0("**Distribution of community temporal trends across sites**: (a) total
  ...
  site and the dots the average posterior distribution with
  the bars depicting the Bayesian credible intervals at 95\\%.
  The dashed lines refer to no temporal trend.", " N = ", fnop, " sampling
  events across over ", fnsite, " sites.") %>%
  str_replace_all("\\s+", " ")
...
site levels (see Methods). We found strong evidence for an average increase over
time in total community abundance (average Credible Interval 95%:
`r abun_trends` per decade, Fig. 1a) and in species richness (CI 95%:
`r rich_trends` per decade, Fig. 1b). By contrast, we found an average decline
in the proportion of non-native species abundance (CI 90%:
`r exo_trends90[["perc_exo_abun"]]` per decade, Fig. 1c), and no evidence for a
trend in the proportion of non-native species richness (CI 80%:
`r exo_trends80[["perc_exo_sp"]]` per decade, Fig. 1d). We also found strong
evidence for rapid community reorganisation, with an average decline in
community similarity of about 30% per decade when considering either species
abundances (Simpson dissimilarity, CI 95%: `r hill_trends` per decade, hereafter
“temporal dissimilarity”, Fig. 1e) or occurrences (Jaccard dissimilarity, CI
95%: `r jac_trends` per decade, Fig. S2a). The close average trends between
Simpson and Jaccard dissimilarity indicated that changes in temporal
dissimilarity resulted from changes in the identity of the dominant rather than
of the rare species. The partitioning of the Jaccard dissimilarity index into
turnover, describing composition changes arising from species replacement, and
nestedness, describing changes arising from species gains and losses from a
common species pool, further showed a comparable increase over time (CI 95%:
`r tu_trends` and `r ne_trends` per decade respectively; Fig. 1f and S2b). This
suggests that changes in community composition were driven by species
replacement in the community, in addition to species losses and gains.

Beyond these general patterns, we also uncovered an important heterogeneity in
the biodiversity temporal trends across local communities (Fig. 1, see also
Supplementary Map S1). The high variability of community changes across space
was confirmed by the hierarchical spatial structure of the models (i.e. random
effects) for which the variance in the temporal community trends was found to be
up to twice higher among sites than among river basins for all community metrics
(Table S1). This indicated that temporal trends were more varied within than
among basins.
...
pca_clust_cap <- paste0("**Covariation among the temporal trends in community metrics and
  ...
  fnop, " sampling events across over ", fnsite, " sites.") %>%
str_replace_all("\\s+", " ")
...
species richness (PC1 & PC2 explaining `r sum(var_exp[1:2])`% of the total
variability among fish communities; Fig. 2a). In addition, temporal trends in
total abundance and species richness tended to be negatively associated, as did
temporal trends in temporal dissimilarity and turnover (PC3 & PC4 explaining
`r sum(var_exp[3:4])`% of the variability among fish communities; Fig. 2b). We
further detected six distinct types of community trajectories (Fig. 2c) using a
k-mean trimmed clustering method (see Methods). The largest cluster was
...
dissimilarity and turnover (‘medium change’; `r per_cluster[1]` of the sites). The second
cluster was associated with communities displaying a
strong turnover but moderate increases in total abundance, species richness and
temporal dissimilarity (‘high turnover’; `r per_cluster[2]` of the sites). The third, fourth
and fifth clusters were characterised by temporal community changes along a
single dimension: a strong increase in species richness (‘increase in species
richness’; `r per_cluster[3]` of the sites), a strong decline in total abundance (‘decrease in
total abundance’; `r per_cluster[4]` of the sites) or a strong decline in species richness
(‘decrease in species richness’; `r per_cluster[5]` of the sites), respectively. The last and
smallest cluster was associated with communities that remained relatively stable
over time (‘low change’; `r per_cluster[6]` of the sites). Although some variability was
apparent across river basins (see Supplementary Map S1), the hierarchy of the
different community trajectories was conserved across the three main
biogeographic realms with similar frequencies (Fig. 2d).
...
as revealed by the consideration of additional covariates to the previous models
(Fig. 3a, see model predictions in Fig. S4). Specifically, we found strong
evidence that a higher degree of past anthropogenic pressures (i.e. human
footprint index of 1993 corresponding to the beginning of the time series) was
associated with faster increases in total abundance and species richness (resp.
CI 95%:$\beta\prime=$ `r hft93abun` in blue and `r hft93rich` in green, Fig.
3a). We also uncovered evidence for an interaction with the longitudinal stream
gradient (i.e. PCA axis with high values associated with a higher Strahler
order, a higher distance from source, and a higher annual discharge), such as
the legacy effects of past anthropogenic pressures on total abundance (strong
evidence) and species richness (weak evidence) were being buffered in more
downstream areas (resp. CI 95%:$\beta\prime =$ `r rivhftabun` and CI
80%:$\beta\prime =$ `r rivhftrich`, Fig. 3a). We also found evidence (albeit
weak) that a higher degree of past anthropogenic pressures was associated with
an increase in the proportion of non-native richness over time (CI
...
3a). Although we did not find evidence for an overall effect of past
anthropogenic pressures on the temporal trend in non-native species abundance
...
areas (CI 90%:$\beta\prime =$ `r rivhftexoa`, Fig. 3a). We also found strong
evidence that a higher degree of past anthropogenic pressures was associated
with a faster rate of increases in temporal dissimilarity and turnover (resp. CI
95%:$\beta\prime =$ `r hft93hill` in red and `r hft93turn` in purple, Fig.
3a), irrespective of the position along the stream gradient (resp. CI
80%:$\beta\prime =$ `r rivhfthill` and `r rivhftturn`, Fig. 3a). Similar results
were obtained when using an alternative estimate of species richness and for
Jaccard instead of Simpson-based dissimilarity (Fig. S5).
...
species richness (CI 80%:$\beta\prime =$ `r hft09rich`, Fig. 3a) but the moderate
evidence for a positive interaction with the longitudinal stream gradient (CI
90%:$\beta\prime =$ `r rivhft09rich`, Fig. 3a) indicated that recent increases in
anthropogenic pressures were associated with faster increases in species
richness over time in more downstream areas. We also found moderate evidence
that a recent increase in anthropogenic pressures was associated with a higher
increase in the proportion of non-native species abundance over time in more
downstream areas (CI 90%:$\beta\prime =$ `r rivhft09exoa`, Fig. 3a), although
recent changes in anthropogenic pressures had no overall effect (CI
80%:$\beta\prime =$ `r hft09abun`, Fig. 3a). When considering the share of
non-native species richness, we found moderate evidence that a recent increase
in anthropogenic pressures was associated with an increase in the proportion of
non-native species (CI 90%:$\beta\prime =$ `r hft09exor`, Fig. 3a). This effect
was particularly pronounced in the historically most degraded sites and in the
most downstream areas (resp. CI 90%:$\beta\prime =$
...
$\beta\prime =$ `r hft09hill` and `r hft09turn`, Fig. 3a). This effect was
hastened in the most historically degraded sites (resp. CI 95%:$\beta\prime =$
`r hft2hill` and `r hft2turn`, Fig. 3a), but not affected by the position of the
sites along the stream gradient (resp. CI 80%:$\beta\prime =$
`r rivhft09hill` and `r rivhft09turn`). By contrast, our results indicate that a
recent decrease in anthropogenic pressures in the historically most degraded
sites is associated with higher rates of increase in total abundance and species
richness, together with a decline in non-native richness and lower rates of
temporal dissimilarity and turnover (see model predictions in Fig. S4).


```{r}
fig1_cap <- paste0("**Drivers of community temporal changes and spatial
  variations**. Effects of anthropogenic pressures and stream gradient on (a)
  community temporal changes and (b) community spatial variations. Community
  ...
  fnop," sampling events over ", fnsite," sites.") %>%
str_replace_all("\\s+", " ")
...
time, Fig. 3b). We found that a higher degree of past anthropogenic pressures
was associated with lower total abundance (strong evidence; Fig. 3b, blue), with
the most ‘degraded’ sites (i.e. with a human footprint index =
`r round(pred_data_explanation$hft_ix_c93["max"],1)`) displaying a total
abundance `r abun_rich_hft93_int_max["log_total_abundance"]` lower than the most
‘intact’ sites (i.e. with a human footprint index =
...
species richness (strong evidence; Fig. 3b, green), with the most degraded sites
displaying +`r abun_rich_hft93_int_max["log_chao_richness"]` more species than
the most intact sites. Sites that had experienced a higher degree of past
anthropogenic pressures also had a higher proportion of non-native abundance and
non-native richness (strong evidence; Fig. 3b, orange and yellow), such as the
most degraded sites had a proportion of non-native species abundance three times
higher than intact sites (`r scales::percent(pred_hft93_t0$perc_exo_abun$val["max"])` versus
`r scales::percent(pred_hft93_t0$perc_exo_abun$val["intact"])`), and a proportion of
non-native species richness twice higher (`r scales::percent(pred_hft93_t0$perc_exo_sp$val["max"])` versus
...
higher proportion of non-native species abundance
(+`r other_hft9309_0_inc_by2["perc_exo_abun"]`) and richness
(+`r other_hft9309_0_inc_by2["perc_exo_sp"]`).
The longitudinal stream gradient was also associated with marked spatial
...
(+`r other_riv_int_max["perc_exo_abun"]`) and non-native richness
(+`r other_riv_int_max["perc_exo_sp"]`). Finally, the models indicated that a
larger proportion of the variance in the community metrics was captured by site
and basin identity rather than by the fixed effects alone (R² conditional
varying from 0.15 for turnover to 0.80 for species richness versus R² marginal
varying from 0.02 for species richness to 0.07 for community turnover; Table 1).
...
declines reported at the global scale for freshwater species and particularly
fish megafauna, as well as with other regional assessments of fish
assemblages[@he_global_2019];@reid_emerging_2019]. We further found a faster
average trend in Jaccard dissimilarity (+31% versus +10% per decade) but a
slower average turnover (+17% versus +28% per decade) than previously reported
across a diversity of marine, freshwater and terrestrial
assemblages[@dornelas_assemblage_2014; @blowes_geography_2019]. This suggests
that riverine fish communities are experiencing both strong changes in species
richness and in community composition.

Our results indicate that these contrasted community changes could be traced
back to the spatio-temporal changes in human pressures. Indeed, higher past
anthropogenic pressures were associated with faster rates of species richness
...
legislations targeting improvements in water quality in the EU and the USA
...
areas. This indicates that non-native species contributed significantly to the
temporal changes observed in riverine fish communities. It follows that the
...
anthropogenic pressures was also associated with faster rates of species
replacement and changes in species dominance over time. This suggests that the
legacy effects of past habitat degradation are characterised by shifts towards
species that are better adapted to degraded environments[@chen_threshold_2020],
to which non-native species contribute
disproportionately[@toussaint_historical_2014; @kuczynski_concomitant_2018].
This interpretation is consistent with previous hypothesis that rapid changes in
terrestrial, marine and freshwater assemblages could be underpinned by the
...
Importantly, we uncovered interactions between past and recent anthropogenic
pressures in driving community changes, highlighting the importance of
considering both the degree and timing of anthropogenic pressures. For instance,
increases in total abundance over time in the most historically degraded sites
were not observed when these sites were also exposed to a recent increase in
anthropogenic pressures. Likewise, the synergy between past and recent
anthropogenic pressures resulted in faster rates of increase in non-native
species richness than expected based on their individual effects.  Conversely,
our results indicate that sites that experienced a recent decrease in
anthropogenic pressures had higher rates of increases in total abundance and
species richness,  higher rates of declines in non-native species richness as
well as slower community composition changes, especially across the most
historically degraded sites. These findings clearly illustrate how a strong
reduction in anthropogenic pressures in highly degraded habitats can lead to a
rapid improvement of riverine fish communities, highlighting opportunities to
restore and maintain surrounding lands to benefit freshwater
biodiversity[@tickner_bending_2020]. In turn, the fact that non-native species
were more abundant in both historically or recently degraded sites but that no
direct associations was uncovered in terms of non-native temporal trends can be
explained, at least in part, by commonly reported time lags between the first
recorded introduction and the establishment of self-sustaining populations,
which includes time for biological acceptance and local adaptation for
example[@bernery_freshwater_2022]. Overall, this demonstrates that recent
habitat degradation can result in simultaneous negative and positive effects on
native and non-natives species, respectively[@mcgill_fifteen_2015;
@dornelas_balance_2019], but highlights the conservation challenges associated
with the identification and management of biodiversity changes in the context of
transient community dynamics[@jackson_balancing_2010].

The position of the sites along the stream gradient was found to mediate
temporal biodiversity trends, with the downstream sites experiencing faster
rates of community changes over time. This finding may be explained by the
higher connectivity of downstream sites with other tributaries, which in turn
gives more opportunity for local colonization[@altermatt_river_2013] and the
establishment of metacommunity dynamics[@tonkin_role_2018]. Likewise, it is not
entirely surprising that community changes were found to be more heterogeneous
at the local scale than at the basin or realm scales and that the spatial
structure of the model explained much more of the variance in the community
metrics than the fixed effects. This may reflect the characteristics of riverine
habitats, and especially their dendritic structure and isolation within drainage
basins, which determine environmental filtering and dispersal
...
withdrawals[@comte_climate_2021; @outhwaite_agriculture_2022]. Our results
further confirm that temporal changes in species composition can be decoupled
from changes in species richness, similarly to what has been observed in other
marine , freshwater and terrestrial assemblages[@dornelas_assemblage_2014;
@blowes_geography_2019]. We uncovered different community trajectories
characterised by changes along only one or several biodiversity dimensions, i.e.
changes in species richness, total abundance, turnover or medium/low community
changes over time. Divergent community trajectories likely result from the
complex mosaic in the degree and timing of anthropogenic pressures and position
of the sites within watersheds. For example, the clusters related to temporal
increases in total abundance and species richness likely correspond to sites
historically degraded, whereas the cluster associated with strong turnover may
be related to recent increases in anthropogenic pressures in historically
degraded sites that resulted in slow increase in species richness and total
abundance, but rapid species replacement, due in part to by the establishment of
non-native species. It follows that the similar frequencies in community
trajectories detected across realms together with the restricted number of sites
displaying a low degree of change are likely to reflect the spatially and
temporally heterogeneous patterns in human pressures. Yet,
we recognize that recent increases in anthropogenic pressures are most prevalent
in tropical biodiversity hotspots[@venter_global_2016], while our study has a spatial coverage
limited to historically industrialised countries. Consequently, the overall
increases in species richness and total abundance reported here that are largely
decoupled from changes in community composition might not be universal
phenomena, and more variable community changes and complex community
trajectories are to be expected at the global scale. Nonetheless, our results
suggest that the multidimensionality of community changes can be traced back to
past environmental challenges and habitat structure, calling for further
investigations that merge multiple scales of both time and
space[@mcgill_fifteen_2015; @primack_biodiversity_2018].

In conclusion, our study uncovered the complex associations between community
changes and anthropogenic pressures in riverine fish communities both in time
and space. We showed that the timing of anthropogenic pressures matters, because
past and recent pressures can have contrasting effects on species richness and
abundance, partly mediated by the contributions of non-native species. Our study
further highlights that considering multiple biodiversity facets can help us
understand the complex mechanisms by which communities disassemble over time.
Looking forward, we emphasise the increasing need to investigate biodiversity
changes across spatial scales to better reconcile reported local gains and
global declines in biodiversity[@vellend_plant_2017; @primack_biodiversity_2018;
@mcgill_fifteen_2015]. The increasing availability of community
...
We used the RivFishTime database[@comte_rivfishtime_2021], a compilation of more
than 12,000 time series containing species abundances of riverine fish
...
https://kalkdatabasen.lansstyrelsen.se/)[@svenson_swedish_1995] to avoid
...
totalling `r fnop` sampling events, `r fnmeas` species abundance records, and `r
fnsp` freshwater fish species. The median time span of the time series was of
...
first study year was `r tss$min`. The sites were mostly located in Palearctic
...
(`r per_pays["FRA"]`), Sweden (`r per_pays["SWE"]`), and the USA
...
database documenting species status across drainage basins of the world[@tedesco_global_2017] (`r nat_per["tedesco"]` of the species
...
of the USA, we completed the global database with the Nonindigenous
Aquatic Species (NAS) database developed by the US Geological Survey
(https://nas.er.usgs.gov/) using US states as spatial grain (`r nat_per2["usgs"]`
...
S3). We then estimated the percentage of non-native species for each sampling We
then estimated the percentage of non-native species with respect to both
abundance and species richness for each sampling event (Table S4).
...
modelling from ground data, making it less prone to errors[@venter_global_2016].
The human footprint index ranges from 0 to 50, with values superior to four
being considered in a degraded state[@williams_change_2020]. This index has been
previously related to species extinction and risk of biological
invasion[@di_marco_changes_2018; @williams_change_2020]. To capture both the
...
before 1993, while `r perc_op9309` took place between 1993 and 2009 and `r
perc_opa09` after 2009; Fig. S1). In order to obtain interpretable coefficients
of recent changes in human footprint, we log-transformed the ratio of human
footprint with a base 2, such as values of -1 and 1 represented a division by
two and a multiplication by two of the human footprint between 1993 and 2009,
respectively. In river networks, the environmental heterogeneity and
connectivity along the longitudinal stream gradient (upstream-downstream)
strongly shape species occurrences, immigration rates and community
composition[@altermatt_river_2013]. To capture this stream gradient, we first
described stream characteristics at each site by the altitude (m), slope (deg.),
average annual discharge ($m^3.s^{-1}$), distance from source (km), and Strahler
...
the sites to the closest stream segment using a one-kilometre buffer (99% of the
sites). We next performed a Principal Component Analysis over the site stream
characteristics on the log-transformed (after adding absolute minimum values
plus one to avoid few negative values in altitude: $x^{\prime}_i = x_i +
min(x_i) + 1$) and standardising all the variables, i.e. centring and scaling.
...
representation (i.e. their loadings) on the two first principal components. The
first rotated component was positively related to average annual discharge,
distance from source and Strahler order and captured `r perc_var_rc1` of the
variance (Fig. S6), and was subsequently used as a composite variable describing
the longitudinal stream gradient from upstream to downstream. The correlation
coefficients indicate little covariations among the predictors (maximum
correlation of 0.09 found between past and recent pressures, Fig. S7).
...
the frequency of outliers. There was very multicollinearity in the model, as all
...
of affiliation to the clusters[@fritz_tclust_2012]. We found a few more NAs
in Nearctic realms (+5%, i.e. around 40 clusters over 897, Fig. S3). The
clustering was performed using `tclust` R package[@fritz_tclust_2012].
...
We thank Joacim Näslund  for his assistance accessing the data from the
long-term Swedish liming program. We also thank all the monitoring programs and
...
and generosity. In particular, we would like to acknowledge We further thank
Suzanne Bonnamour, Colin Fontaine, Grégoire Loïs, Maud Mouchet, and Emmanuelle
Porcher at the Centre d’Écologie et des Sciences de la Conservation as well as
Andrew P Beckerman and Thomas F Johnson at the University of Sheffield for their
very constructive feedbacks on the study. AD was supported by NERC Grant
NE/T003502/1.
