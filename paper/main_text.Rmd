...
found evidence of rapid community reorganisation, characterised by a mean
species replacement of 30% per decade with remarkable changes in the identity of
dominant species, together with an increase in total abundance of 13% per
decade, and species richness of 7% per decade. An important spatial
heterogeneity in these trends was mediated by non-native species introductions
and could be traced back to the strength and timing of anthropogenic pressures.
Specifically, we demonstrate that the negative effect of anthropogenic pressures
on species richness and total abundance were compensated over time by the
establishment of non-native species, a pattern consistent with previously
reported biotic homogenization at global scale. Overall, our study suggests
that accounting for the complexity of community changes and its drivers is a
crucial step to reach global conservation goals.
...
@blowes_geography_2019], resulting in ecosystem consequences manifested across
large spatial scales[@primack_biodiversity_2018; @moore_response_2017]. Repeated
calls have been made for scientific clarity in how heterogeneous rates of
species losses and gains across the landscape are driving shifts in community
structure over time[@mcgill_fifteen_2015]. Advancing this knowledge is
...
ecosystems[@reid_emerging_2019] with striking repercussions for fish
...
Non-native fish species can also play a disproportionate role in the
recomposition of communities over time[@leprieur_fish_2008;
@moore_response_2017; @bernery_freshwater_2022; @villeger_homogenization_2011],
and have dramatic effects on native species when they become
invasive[@gallardo_global_2016], resulting in the widespread homogenization of
historically distinct faunas[@olden_homogocene_2018]. Hubs of human activities
such as human settlements, transport and trade are also responsible for major
habitat alterations as well as more frequent non-native introduction events by
increasing accessibility and connectivity among habitats[@chapman_invasion_2020;
@lockwood_role_2005; @gallardo_importance_2015]. Human activities may therefore
have opposing effects on local diversity by decreasing the number and abundance
of native species, while concurrently promoting the establishment and spread of
non-native species that can increase community species richness and total
abundance[@mcgill_fifteen_2015; @primack_biodiversity_2018]. Understanding
community changes therefore requires going beyond analyses of changes in the
number of species or individuals by considering concomitant changes in the
identity of species[@dornelas_assemblage_2014; @blowes_geography_2019;
@scott_native_2001; @hillebrand_biodiversity_2018].
...
may therefore erroneously conclude on the drivers of community changes.
Moreover, spatial connectivity can enhance or dampen community responses to
anthropogenic pressures by modulating the frequency of introduction events. We
tackled this challenge by investigating how, and why riverine fish communities
change spatially and temporally from local to continental extents. To do so,
we leveraged a compilation of 4,476 riverine fish community
timeseries[@comte_rivfishtime_2021] that had been repeatedly sampled over the
last three decades in various river basins across three biogeographic realms. We
quantified temporal changes in total abundance, species richness, and community
composition across local communities, including the temporal changes in the
share of non-native species. We next characterised the typology of community
temporal trends by examining the covariation among different communiy metrics,
and distribution of community trajectories across spatial scales. We finally
assessed how fish community changes could be traced back to the spatio-temporal
changes in anthropogenic pressures and position along the longitudinal stream
gradient. This study aims to further our understanding of the complexity of
local community changes by addressing the effects of anthropogenic pressures and
advancing new knowledge necessary to inform actions seeking to curb the current
...
```{r}
trends_cap <- "Distribution of site temporal trends per year estimated from a
hierarchical Bayesian model including time as sole predictor (See Methods, main
text). Temporal trends were estimated with the Best Linear Unbiased Predictor
(BLUP). Dashed lines display null trends. Below the histograms, points depict
the average of the posterior distribution. Large, medium and thin bars depict
the Bayesian credible intervals at 80, 90 and 95\\%, respectively." %>%
str_replace_all("\n|\r", " ") %>%
str_replace_all("  ", " ")
```
...
series). On average, communities have increased in both total abundance and
species richness, while concurrently experiencing shifts in community
composition caused by changes in species richness, species turnover, and
dominance structure (i.e. identity of the dominant species). We estimated
community temporal trends with a hierarchical Bayesian model including the
number of years since the first survey as sole predictor and accounting for the
spatial variations of temporal trends at the hydrographic river basin and at the
site levels (see Methods).
We found strong evidence for an average increase over time in total community
abundance (average Credible Interval 95%: `r abun_trends` per decade, Fig.
1a) and in species richness (CI 95%: `r rich_trends` per decade, Fig. 1b). By contrast,
we found an average decline in the proportion of non-native species abundance
(CI 90%: `r exo_trends90[["perc_exo_abun"]]` per decade, Fig. 1c), and no evidence for a trend in the
proportion of non-native species richness (CI 80%: `r exo_trends80[["perc_exo_sp"]]` per
decade, Fig. 1d). We also found strong evidence for rapid community reorganisation, with
an average decline in community similarity of about 30% per decade when
considering either species abundances (Simpson dissimilarity, CI 95%: `r hill_trends`
per decade, hereafter “temporal dissimilarity”, Fig. 1e) or occurrences
(Jaccard dissimilarity, CI 95%: `r jac_trends` per decade, Fig. S2a). The partitioning
of the Jaccard dissimilarity index into turnover, describing composition changes
...
increase over time (CI 95%: `r tu_trends` and `r ne_trends` per decade
respectively; Fig. 1f and S2b). This suggests that changes in community composition were
driven by species replacement in the community, in addition to species losses
and gains.
...
select(`Response variable`, `Spatial scale`, `Marg. R2`, `Cond. R2`, `Temporal trends s.d.`) %>%
mutate(`Response variable` = factor(`Response variable`, levels = names(pal_variable))) %>%
arrange(`Response variable`)

tab_rand_cap <- "**Marginal and Conditional R2 of the hierarchical Bayesian model for each
community metric (results in Fig. 3)**. The model accounted for spatial variations at the
hydrographic river basin and site levels as random effects, site effect being
nested in basin effect. Mean R2 [95\\% CI]: Credible Interval computed using the
Highest Posterior Density method. Conditional R2 account only for spatial
variations on the intercept (See Method)." %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
Beyond these general patterns, we also uncovered an important heterogeneity in
the biodiversity temporal trends across local communities (Fig. 1, see supplementary map). The high
variability of community changes across space was confirmed by the hierarchical
spatial structure of the models (i.e. random effects) which captured much of the
variance in the biodiversity metrics (R² conditional on the random intercepts
varying from 0.15 for turnover to 0.80 for species richness; Table 1). We
further found that the variance in the temporal community trends was up to twice
higher among sites than among river basins for all community metrics, indicating
that temporal trends were more varied within than among basins (Table S1).
...
pca_clust_cap <- paste0("**Covariation among the temporal trends in biodiversity
...
boxplots displaying the distribution of the temporal trends by cluster. (d)
Cluster frequencies across the three main realms. The
sites are coloured by their cluster belongings. The ellipses in (a-b) display
the 95\\% intervals around the clusters assuming a Student’s t distribution. The
clusters were named according to their main characteristics.",
" Sites not assigned to a cluster ", na_cluster, " are displayed in Fig. S3.") %>%
str_replace_all("\n|\r", " ") %>%
str_replace_all("  ", " ")
```
...
Our results indicated a low degree of covariation among the temporal trends for
the different community metrics resulting in distinct types of community
trajectories (Fig. 2). Temporal trends in community composition (i.e. temporal
dissimilarity and turnover), as revealed using a Principal Component Analysis
(PCA), were largely independent of temporal trends in total abundance and
species richness (PC1 & PC2 explaining 69% of the total variability among fish
communities; Fig. 2a). In addition, temporal trends in total abundance and
species richness tended to be negatively associated with temporal dissimilarity
and turnover (PC3 & PC4 explaining 31% of the variability among fish
communities; Fig. 2b). We further detected six distinct types of
community trajectories (Fig. 2c) using a k-mean trimmed clustering method (see
Methods). The largest cluster was characterised by moderate changes along all
biodiversity dimensions: medium temporal increases in total abundance and
species richness, temporal dissimilarity
and turnover (‘medium change’; 40% of the sites). The second cluster was
associated with communities displaying a strong turnover but moderate increases
in total abundance, species richness and temporal
dissimilarity (‘high turnover’; 16% of the sites). The third, fourth and fifth
clusters were characterised by temporal community changes along a single
dimension: a strong increase in species richness (‘increase in species
richness’; 13% of the sites), a strong decline in total abundance (‘decrease in
total abundance’; 12% of the sites) or a strong decline in species richness
(‘decrease in species richness’; 9% of the sites), respectively. The last and
smallest cluster was associated with communities that remained relatively stable
over time (‘low change’; 7% of the sites). Different community trajectories were
observed across the three main biogeographic realms with similar frequencies (Fig. 2d),
although some variability was apparent across river basins (see supplementary
map). Remarkably, the hierarchy of cluster frequencies was conserved across
biogeographic realms (Fig. 2d).
...
Our results revealed complex synergies between the legacy of past anthropogenic
pressures and the effects of recent anthropogenic human pressures on community
temporal trends, mediated by non-native species and longitudinal stream position
(Fig. 3, see model predictions in Fig. S4).
Specifically, we found strong evidence that a higher degree of past
anthropogenic pressures (i.e. human footprint index of 1993 corresponding to the
beginning of the time series) was associated with faster increases in total
abundance and species richness (resp. CI 95%:$\beta\prime=$ `r hft93abun` in blue and
`r hft93rich` in green, Fig. 3a). We also uncovered moderate evidence for an
interaction with the longitudinal stream gradient (i.e. PCA axis with high
values associated with a higher Strahler order, a higher distance from source,
and a higher annual discharge), such as the legacy effects of past
anthropogenic pressures on total abundance and species richness increases
being buffered in more downstream areas (resp. CI 80%:$\beta\prime =$ `r rivhftabun`
and CI 95%:$\beta\prime =$ `r rivhftrich`, Fig. 3a). We also found weak evidence
that a higher degree of past anthropogenic pressures was associated with an
increase in the proportion of non-native richness over time (CI 80%:$\beta\prime
=$ `r hft93exor`, yellow, Fig. 3a), and that this effect was
enhanced in more downstream areas (CI 90%:$\beta\prime =$ `r rivhftexor`, Fig.
3a).
Although we did not find evidence for an overall effect of past anthropogenic
pressures on the temporal trend in non-native species abundance (CI
80%:$\beta\prime =$ `r hft93exoa` in orange, Fig. 3a), we found moderate
evidence that a higher degree of past anthropogenic pressures resulted in a
higher rate of increase in
non-native species abundance in the most downstream areas (CI 90%:$\beta\prime
=$ `r rivhftexoa`, Fig. 3a). We found strong evidence that a higher degree of
past anthropogenic pressures was associated with a faster rate of increases in temporal
dissimilarity and turnover (resp. CI 95%:$\beta\prime =$ `r hft93hill` in red
and `r hft93turn` in purple, Fig. 3a), irrespective of the position along the
stream gradient (resp. CI 80%:$\beta\prime =$ `r rivhfthill` and `r rivhftturn`,
Fig. 3a). Further analysis indicated that changes in temporal dissimilarity
involved changes in the identity of the dominant species (Fig. S5).
...
total abundance and species richness as well as hasten community reorganisation
through increases in the share of non-native species and faster rates of
temporal dissimilarity and turnover (Fig. 3a, see model prediction in Fig. S4).
More specifically, we found strong evidence of an antagonistic effect between
past and recent anthropogenic pressures on total abundance (CI 95%:$\beta\prime
=$ `r hft2abun`, Fig. 3a), such
...
Recent anthropogenic pressures per se had no effect on the temporal trends in
total abundance (CI 80%:$\beta\prime =$ `r hft09abun`, Fig. 3a). Similarly, recent
changes in anthropogenic pressures did not have an overall effect on the
temporal trends in species richness (CI 80%:$\beta\prime =$ `r hft09rich`, Fig. 3a).
However, the moderate evidence for a positive interaction with the longitudinal
stream gradient (CI 90%:$\beta\prime =$ `r rivhft09rich`, Fig. 3a) indicated
that recent increases in anthropogenic pressures were associated with faster
increases in species richness over time in the most downstream areas. We also
found moderate evidence that a recent increase in anthropogenic pressures
resulted in a higher increase in the proportion of non-native species abundance
over time in the most downstream areas (CI 90%:$\beta\prime =$ `r rivhft09exoa`,
Fig. 3a),
although recent changes in anthropogenic pressures had no overall effect (CI
80%:$\beta\prime =$ `r hft09abun`, Fig. 3a). When considering the share of non-native
species richness, we found moderate evidence that a recent increase in
anthropogenic pressures was associated with an increase in the proportion of
non-native species (CI 90%:$\beta\prime =$ `r hft09exor`, Fig.
...
`r hft2exor` and `r rivhft09exor`, Fig. 3a). In addition, we found strong
evidence that a recent increase in anthropogenic pressures resulted in stronger
temporal dissimilarity and turnover (resp. CI 95% $\beta\prime =$ `r hft09hill`
and `r hft09turn`, Fig. 3a). This effect
was hastened in the most historically degraded sites (resp. CI 95%:$\beta\prime =$
`r hft2hill` and `r hft2turn`, Fig. 3a), but not affected by the position of
the sites along the stream gradient (resp. CI 80%:$\beta\prime =$
`r rivhft09hill` and `r rivhft09turn`). On the hand, our results also suggest that a recent
decrease in anthropogenic pressures in the historically most degraded sites
result in higher rates of increase in total abundance and species richness,
together with a decline in non-native richness and lower rates of temporal
dissimilarity and turnover (see model predictions in Fig. S4).


```{r}
fig1_cap <- paste0("**Drivers of community temporal changes and spatial variations**.
  (a) effects of anthropogenic pressures and stream gradient on temporal
  changes and (b) spatial variations. Community metrics include total
  abundance, species richness, relative abundance and richness of non-native species,
  Simpson temporal dissimilarity, and Jaccard turnover. Points
  depict the average of the posterior distributions. Large, medium and thin bars
  depict the Bayesian credible intervals at 80, 90 and 95\\%, respectively.
  Please note the broken abscissa scale in the framed panels. N = ", fnop,"
  sampling events over ", fnsite," sites.") %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
Our results indicated strong variation in community properties across space
associated with past and recent anthropogenic pressures as well as position
along the longitudinal stream gradient (i.e. single model effects independent of
time, Fig. 3b). We found that a higher degree of past anthropogenic pressures was
associated with lower total abundance (strong evidence; Fig. 3b, blue), with the
most ‘degraded’ sites (i.e. with a human footprint index =
`r round(pred_data_explanation$hft_ix_c93["max"],1)`) displaying a
total abundance `r abun_rich_hft93_int_max["log_total_abundance"]` lower than
the most ‘intact’ sites (i.e. with a human footprint index =
`r round(pred_data_explanation$hft_ix_c93["intact"],1)`). In addition,
we found that a higher degree of past
anthropogenic pressures was associated with higher species richness (strong
evidence; Fig. 3b, green), with the most degraded sites displaying
+`r abun_rich_hft93_int_max["log_chao_richness"]` more
species than the most intact sites. Sites that had experienced a higher degree
of past anthropogenic pressures also had a higher proportion of non-native
abundance and non-native richness (strong evidence; Fig. 3b, orange and yellow),
such as the most degraded sites had a proportion of non-native species abundance
three times higher than intact sites
(`r scales::percent(pred_hft93_t0$perc_exo_abun$val["max"])` versus
`r scales::percent(pred_hft93_t0$perc_exo_abun$val["intact"])`), and a proportion of non-native
species richness twice higher (`r scales::percent(pred_hft93_t0$perc_exo_sp$val["max"])` versus
`r scales::percent(pred_hft93_t0$perc_exo_sp$val["intact"])`). By contrast, we found
strong evidence that recent increases in anthropogenic pressures were associated
with communities displaying on average lower total abundance and species
richness but an increased share of non-native species (Fig. 3b). More
specifically, sites that had been subjected to a recent two-fold increase in
anthropogenic pressures displayed total abundance
`r abun_rich_hft9309_0_inc_by2["log_total_abundance"]` and species richness
`r abun_rich_hft9309_0_inc_by2["log_chao_richness"]`
lower compared to sites that had not undergone recent degradations, but a much
higher proportion of non-native species abundance (+`r other_hft9309_0_inc_by2["perc_exo_abun"]`)
 and richness (+`r other_hft9309_0_inc_by2["perc_exo_sp"]`). The
longitudinal
stream gradient was also associated with marked spatial
variation. The most downstream sites displayed three times more species than the
most upstream sites, as well as a higher proportion of non-native abundance
(+`r other_riv_int_max["perc_exo_abun"]`)
and non-native richness (+`r other_riv_int_max["perc_exo_sp"]`).

# Discussion

Recent decades have witnessed substantial shifts in fish riverine communities
characterised by marked increases in species richness and total abundance over
time, accompanied by a strong pattern of species replacement. Interestingly,
...
the Palearctic, such as the reported increase in freshwater insect occupancy
...
megafauna as well as with other regional assessments of fish
assemblages[@he_global_2019]. We further found a higher average trend in Jaccard
dissimilarity (+31% versus +10% per decade) but a lower average turnover (+17%
versus +28% per decade) than previously reported in marine, freshwater and
terrestrial assemblages[@dornelas_assemblage_2014; @blowes_geography_2019].
Those contrasting findings could be traced back to that we report rapid changes
due to changes both in species richness and in community composition while
previous studies found changes stemming mainly from community composition
changes.

Our results suggest that these contrasted community changes could be traced back
to the spatio-temporal changes in human pressures. Indeed, higher past
anthropogenic pressures were associated with higher rates of species richness
and total abundance increases over time, suggesting a recovery from the legacy
of past disturbances. Previous studies suggested that the adoption of numerous
legislations targeting improvements in water quality in the E.U. and the U.S.
since the 1970s, as well as a decrease in the negative effects of agriculture
could be partly responsible for those increases despite the surrounding habitat
changes[@langford_long-term_2009; @van_klink_meta-analysis_2020;
@outhwaite_agriculture_2022]. The fact that most of our monitored sites (92%) could be
considered in an initial state of degradation at the beginning of the study
period, i.e. had a human footprint index > 4 in 1993[@williams_change_2020],
could therefore lend support to the recovery hypothesis.
...
between introduction and extirpation from ongoing spatial
homogenization[@villeger_homogenization_2011], a pattern well supported by
metacommunity models[@mouquet_community_2003] and already documented across
...
number and the frequency of introduction events[@anas_drivers_2021;
...
native species that are less suited to the new
conditions[@bernery_freshwater_2022; @byers_impact_2002;
@olden_life-history_2006; @haubrock_two_2021]. A higher degree of past
anthropogenic pressures also resulted in higher rates of species replacement and
changes in species dominance over time, suggesting that the legacy effects of
past habitat degradation are characterised by shifts in dominant species and
species identity towards species that are better adapted to degraded
environments[@chen_threshold_2020], to which non-native species contribute
disproportionately[@toussaint_historical_2014; @kuczynski_concomitant_2018].
Our results then support the hypothesis that rapid community changes
in marine, freshwater and terrestrial assemblages are unpinned by the
replacement of species by widespread generalists with a significant contribution
of non-native species[@dornelas_assemblage_2014; @blowes_geography_2019].

---
#However, a more definitive interpretation of those
...
#characteristics such as their degree of tolerance or affinity to anthropogenic
#habitats and their habitat niche width, may provide a more direct assessment of
#how the degree of ubiquity and generalism changed over time in these
#communities.
---
...
degree and timing of anthropogenic pressures. For instance, increases in total
abundance over time in the most historically degraded sites were not observed
when they were also exposed to a recent increase in anthropogenic pressures. In
addition, our results show that past and recent anthropogenic pressures drive
synergetically faster rates of increase in relative richness of non-native
species. Overall, this study reveals that recent habitat degradation can result
in simultaneous negative and positive effects on native and non-natives species
respectively[@mcgill_fifteen_2015; @dornelas_balance_2019]. We recognize that
recent increases in anthropogenic pressures are most prevalent in tropical
biodiversity hotspots[@venter_global_2016] while our study has a spatial
coverage limited to historically industrialised countries. Consequently, our
findings suggest that increases in species richness and total abundance might
not be universal phenomena and that more variable community changes are to be
expected at the global scale[@leprieur_fish_2008; @he_global_2019]. However, our
results also indicate that sites that experienced a recent decrease in
anthropogenic pressures had higher rates of increases in total abundance and
species richness while having higher rates of declines in relative richness of
non-native species and slower community changes, especially in the most
historically degraded and more downstream sites. Then, our results suggest that
a strong reduction of anthropogenic pressures in highly degraded habitats can
lead to a rapid improvement of native riverine fish communities.

Patterns in community variation over space revealed that the most historically
degraded sites supported higher species richness compared to sites experiencing
more recent anthropogenic pressures. However, sites on both ends of this
degradation spectrum displayed lower total abundance. These results agree with
previous findings that land degradation results in lower species richness and
total abundance over space[@newbold_global_2015;
@outhwaite_agriculture_2022]. Furthermore, while past and recent changes in
anthropogenic pressures had no associations with temporal trends in the
proportion of non-native species abundance, non-native species were relatively
more abundant in both historically or recently degraded sites. This pattern can
be explained, in part, by commonly observed time lags between the first recorded
introduction of non-native species, and the establishment of self-sustaining
populations, which includes time for biological acceptance and local adaptation
for example[@bernery_freshwater_2022].

Our results confirm that temporal changes in species composition may be
...
@blowes_geography_2019]. We uncovered different community
trajectories characterised by changes along only one or several biodiversity
dimensions, i.e. changes in species richness, total abundance, turnover or
medium/low community changes over time. Divergent community trajectories likely
...
resulted in slow increase in species richness and total abundance, but rapid
...
traced back to past environmental challenges, calling for further investigations
that merge multiple scales of both time and space.
...
more opportunity for local colonization[@altermatt_river_2013] and the
establishment of metacommunity dynamics[@tonkin_role_2018]. Position in the watershed was also
found to mediate the effect of
anthropogenic pressures, decreasing or enhancing temporal changes depending on
the community metric considered. It is thus not surprising that community
...
in turn determine environmental filtering and dispersal opportunities[@dias_global_2014;
@olden_conservation_2010].
Furthermore, we found that the spatial structure of the model explained much
more of the variance in the community metrics than the fixed effects, confirming
previous findings[@scott_native_2001]. It might be related to differences in community composition
arising from climatic factors and the coarseness of anthropogenic pressures, as
specific threats such as climate change and land-use which interact to drive
community reorganisation[@comte_climate_2021; @outhwaite_agriculture_2022].
...
contrasting effects on species richness and abundance, partly mediated by the
contributions of non-native species. Looking forward, we emphasise the
increasing need to investigate biodiversity changes across spatial scales to
better reconcile reported local gains and global declines in
biodiversity[@vellend_plant_2017; @primack_biodiversity_2018;
@mcgill_fifteen_2015]. Our study further highlights that considering multiple
biodiversity facets can help us understand the complex mechanisms by which
communities disassemble over time. The increasing availability of community
timeseries and of environmental data across large areas is therefore invaluable
to understand how human pressures have affected and are persisting to affect
...
completed with time series from Canada and the United States of America (Table
S2). The final database mainly covered western and northern Europe, northern
America, and southeastern Australia. We selected time series having at least 5
years of data over a 10-year period as well as a consistent sampling protocol
and abundance unit. As several sites had been sampled using different sampling
methods (e.g. electrofishing, seining, Table. S3) and/or over different periods
of the year, we selected for each site only the sampling events that were
performed using the most frequent protocol (i.e. the mode) and within 1.5 month
of the most frequently sampled month (i.e. 45 days before or after). When there
were several sampling events in a given year, we selected the sampling that took
place at the closest date from the most frequently sampled date of the site.
Finally, we excluded 1,340 sites that had been limed as part of the long-term
Swedish liming program[@svenson_swedish_1995] to avoid including sites whose
environmental conditions had been experimentally manipulated. To do so, we
used...(**Cf Lise for the database reference**).
...
`r tss$span` years ([25th quantile, 75th quantile]), their median completeness was of
`r tss$completeness` (see Fig. S1 for the complete distribution), while the median
first study year was `r tss$min`. The sites were mostly located in
(`r per_eco["Palearctic"]`), Nearctic (`r per_eco["Nearctic"]`) and
Australasia (`r per_eco["Australasia"]`). Four countries gathered
85% of the sites, namely Great Britain (`r per_pays["GBR"]`), France
...
biodiversity facets related to total abundance, species richness, non-native
...
(CPUE, `r u_per["CPUE"]`), and Leslie index (`r u_per["Leslie_index"]`, Table S3).
...
estimates were high correlated (Spearman’s = 0.97 for both raw variables and log
...
occurrences, Table S3). For the sites falling outside the river basins provided in the
...
of the U.S.A., we completed the global database with the Nonindigenous
...
in the U.S.A. (`r nat_per["handmade"]` of the species occurrences, Table
S3). We then estimated the percentage of non-native species for each sampling
event, both for abundances and species richness (Table S4).
...
index[@venter_global_2016; @venter_sixteen_2016], extracted from the HydroAtlas
database at the reach scale[@linke_global_2019; @robinson_earthenv-dem90_2014]
(reach length resolution of 450 metres). The human footprint index aggregates
an array of human pressures, including population density, the extent of urban,
forested, cropland and pastureland areas, but also transportation hubs such as
roads, railways, and navigable pathways. It does so by combining remote sensing
data, systematic surveys and modelling from ground data, making it less prone to
errors[@venter_global_2016]. The human
...
species extinction and risk of biological invasion[@di_marco_changes_2018;
@williams_change_2020]. To capture both the
...
in anthropogenic pressures (only `r perc_opb93` of the samplings took place before 1993,
while `r perc_op9309` took place between 1993 and 2009 and `r perc_opa09` after 2009; Fig. S1). In
order to obtain interpretable coefficients of recent changes in human footprint,
we log-transformed the ratio of human footprint with a base 2, such as values
of -1 and 1 represented a division by two and a multiplication by
two of the human footprint between 1993 and 2009, respectively. In river
networks, the environmental heterogeneity and connectivity along the
longitudinal stream gradient (upstream-downstream)
strongly shape species occurrences, immigration rates and community
composition[@altermatt_river_2013]. To capture this stream gradient, we first described stream
characteristics at each site by the altitude (m), slope (deg.), average annual
discharge ($m^3.s^{-1}$), distance from source (km), and Strahler order that we
extracted from the HydroAtlas database at the reach scale[@linke_global_2019;
@robinson_earthenv-dem90_2014]. We did so by snapping the sites to the closest
stream segment using a one-kilometre buffer (99% of the sites). We next
performed a Principal Component Analysis over the site stream characteristics on
the log-transformed (after adding absolute
minimum values plus one to avoid few negative values in altitude:
$x^{\prime}_i = x_i + min(x_i) + 1$) and standardising all the variables, i.e. centring and scaling.
We orthogonally rotated the two first principal components using the varimax
criterion[@kaiser_varimax_1958], to increase the quality of the variable representation (i.e. their
loadings) on the two first principal components. The first rotated component was
positively related to average annual discharge, distance from source and
Strahler order and captured `r perc_var_rc1` of the variance (Fig. S6), and was subsequently
used as a composite variable describing the longitudinal stream gradient from
upstream to downstream. The correlation coefficients indicate little
covariations among the predictors (maximum correlation of 0.09 found between
past and recent pressures, Fig. S7).
...
To estimate community temporal community trends, we first modelled the different
community metrics (Y) as dependant of time ($\beta_0Time_t$, eq. \@ref(eq:gen)),
measured as the number of years since the beginning of the sampling at each
site. The statistical model (eq. \@ref(eq:gen)) was adapted according to the
nature of the response variable for the models including total abundance and
dissimilarity metrics. For total abundance, we added the measurement unit of
abundance as a categorical variable both as a main effect and in interaction
with time[@van_klink_meta-analysis_2020]. We set ‘raw count’ as the reference
factor level such as the temporal trends in total abundance in the main text and
supplementary materials are expressed in raw count (i.e. number of individuals).
For dissimilarity metrics, we set the intercept fixed at zero as dissimilarity
...
the intercept ($\alpha$) and on the slope of the temporal trends ($\beta_0$)
conditional on basin identity ($n$) and on site identity ($i$) nested in basin
($i|n$). The random effects and the error terms were modelled as a Normal
...
To assess the drivers of community change, we then incorporated
additional covariates into the models (eq. \@ref(eq:drivers)): the legacy of past anthropogenic
pressures measured by the human footprint index of 1993, the recent changes in
anthropogenic pressures measured by the ratio between the human footprint index
of 2009 and 1993 ($X_k$), and the longitudinal stream gradient estimated by the
rotated PCA axis. We included interactions between time and the ecological
drivers ($\sum_{k = 1}\beta_{0k}Time_tX_k$) to test how stream gradient and anthropogenic pressures
affect the temporal trends in community metrics. We further included
the triple interactions between time and the pairs of other ecological drivers
($\sum_{k = 1, k \neq l}\beta_{0kl}Time_tX_kX_l$) to test for the presence of
synergistic or antagonistic effects between anthropogenic pressures and
longitudinal stream gradient on the temporal trends in community metrics
biodiversity facets. We included all the predictors as main effects
($\sum_{k=1}\beta_kX_k$) to capture the effects of the ecological
drivers on the spatial variations in community composition resulting
from the ecological drivers, except when modelling the dissimilarity metrics
(i.e. we only included the effects of ecological drivers on the temporal
trends). We did so because dissimilarity metrics were solely estimated along the temporal
dimension and are relative to the site (i.e. bounded by 0 and 1 at each site),
and we thus did not expect average differences in dissimilarity related to any
other factors than time.

\begin{equation}
\begin{aligned}
    Y_{i|n, t} &= \alpha + \beta_0Time_t + \sum_{k=1}\beta_kX_k + \sum_{k=1, k}\beta_{0k}Time_tX_k + \sum_{k=1, k \neq l}\beta_{0kl}Time_tX_kX_l + \epsilon_{i|n, t}
  (\#eq:drivers)
\end{aligned}
\end{equation}
...
coefficients across all community metrics facets (e.g. temporal
...
standard deviation prior to model fitting. As our models contain interactions,
...
the $X_k = 0$. Hence, we centred past anthropogenic pressures and stream
gradient around their average values. The variables quantifying time and recent
changes in anthropogenic pressures were not centred, because then the main
effects of the ecological predictors ($\sum_{k=1}\beta_k$) would indicate their
baseline effects (i.e. when time is equal to 0 and without recent changes in
...
coefficients to to the standard deviations attributed to the random effects and
...
We checked the overall quality of the model fit to the data by plotting the fitted versus the
...
to assess both the quality of data representation, and the frequency of outliers.
There was very multicollinearity in the model, as all Variance Inflation Factors
were around 1 (Table S6).
...
the priors[@gelman_r-squared_2019]. As we computed the variance of
...
using the Best Linear Unbiased Prediction method). We exclude the variables
describing the non-native species composition because their predicted temporal
trends at the site level displayed a very low variation compared to the other
variables, which in turn exerted a disproportionate constraint on the analysis.
In addition, we performed a clustering analysis to identify distinct types of
community trajectories using the trimmed k-means method[@fritz_tclust_2012], a
robust clustering method because it avoids the identification of spurious
clusters. The method consists of trimming the $\alpha$ most outlying data in the
multidimensional space, the number of dimensions being the number of
community metrics. To choose a relevant number of clusters, we
plotted the log-likelihood of the trimmed classification as a function of the
proportion of the most outlying data trimmed ($\alpha$) and the number of
clusters (Fig. S9). We thus
selected a partition of temporal community changes in six clusters with
$\alpha=5\%$ (See Fig. S10 with four clusters). We did not constraint the
algorithm for the relative size or shape of the clusters, as we had no a priori
expectation about them. The clustering algorithm was run for a minimum number of
one hundred iterations and up to 125. To further control for the of cluster
assignment, we discarded any fish community for which the second-best cluster
assignment was 50% better than the first one, we did so by comparing the degree
of affiliation to the clusters[@fritz_tclust_2012]. The clustering was performed
using `tclust` R package[@fritz_tclust_2012].
...
We thank [**NAME**] for providing the spatial location of the rivers included in
the Swedish liming program. We thank the agencies and the researchers who
contributed their riverine fish community timeseries. We acknowledge that this
study would not be possible to achieve without their efforts in data collection
and their generosity in contributing their data. We further thank Suzanne
Bonnamour, Colin Fontaine, Grégoire Loïs, Maud Mouchet, and Emmanuelle Porcher
at the Centre d'Écologie et des Sciences de la Conservation as well as Andrew
Beckerman and Thomas F Johnson at University of Sheffield for their deep
insights and constructive feedbacks on the study.
