...
between 1993 and 2019 with a minimum of 5 years of sampling, Fig. S1), mainly
...
`r tm$span["max"]` years [median = `r tm$span["median"]`]; see Fig. S1 for more
...
completeness and starting year (Fig. S2). From this model, we considered weak,
...
S3a). The consistency in the Simpson and Jaccard dissimilarity metrics indicated
...
1f and S3b). This suggests that changes in community composition were driven by
...
heterogeneity is also apparent within the same river basin (Supplementary Map
...
metrics (Table S1). This suggested that relatively finer-scale environmental
...
pca_clust_cap <- paste0("**Covariation among the community temporal trends and
  ...
  displaying the distribution of the temporal trends by cluster. The boxplot
  limits display 1.5 interquartile range. (d) Cluster frequencies across the
  ...
  affiliation uncertainty ", na_cluster, " are displayed in Fig. S4.", " N = ",
...
Fig. S6). In addition, we found that these additional predictors were not
related to the characteristics of the time series (Fig. S5). Specifically, we found strong
...
or coverage-based species richness (See Methods, Fig. S7).
...
Jaccard and Simpson-based dissimilarity (Fig. S7) but the effects of past
...
Fig. S6). More specifically, we found strong evidence of an antagonistic effect
...
prediction (i.e. at $t = 0$, Fig. S6, see Methods), we found that a higher
...
methods (e.g. electrofishing, seining, Table S3), we selected for each site only
...
`r tss$completeness` (see Fig. S1), and the median first study year was
...
`r perc_opa09` after 2009; Fig. S1). We chose the human footprint index
...
of the variance (Fig. S8), and was subsequently used as a composite variable
...
correlation of 0.09 found between past and recent pressures, Fig. S9).
...
fitted versus observed values (Fig. S10). We visually inspected the PIT and
...
as all Variance Inflation Factors were around 1 (Table S6).
...
outlying data trimmed ($\alpha$) and the number of clusters (Fig. S11). We
thus selected a partition of temporal community changes in six clusters with
$\alpha=5\%$ (see Fig. S12 with four clusters). We did not constrain the algorithm for the
...
to date. The data used in the study are open access, although we provide the raw
...
study (Table S2). We further thank Suzanne
