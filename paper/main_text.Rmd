...
timeseries (93% in 1993-2019) across Palearctic, Neartic and Australasia realms,
...
composition of 30% per decade characterized by significant changes in the
...
the effects of invasive species[@strayer_understanding_2006].
...
river basins across mainly Palearctic, Nearctic and Australasia realms (99,9% of
...
Methods) and has been previously related to species extinction and risk of
biological invasion[@anas_drivers_2021; @di_marco_changes_2018]. Outcomes of
this study further our understanding of the complexity of local community
...
trends_cap <- paste0("**Distribution of community temporal trends across sites**: (a) total
  ...
  hierarchical Bayesian model including time as sole fixed predictor (See Methods). The
...
hydrographic river basin and site levels on both intercept and temporal trends
(see Methods for a detailed description of the models), we found that
communities have increased in both total abundance and species richness, but
decreased in the proportional abundance of non-native species in recent decades,
and that the estimated temporal trends were not influenced by the
characteristics of the timeseries (Fig. S2). From this model, we considered
weak, medium and strong evidence for an effect when its confidence interval at
respectively 80, 90, and 95% did not overlap 0 for confidence intervals. We
...
pca_clust_cap <- paste0("**Covariation among the community temporal trends and
  ...
  clusters were named according the most noticeable characteristic of changes
...
We further assessed associations among community metrics temporal trends to
identify community temporal trajectories using the estimated temporal trends in
community composition at the site level from the hierarchical Bayesian model.
There was a moderate level of association among the different community
...
of trajectories appeared orthogonal to each other (Fig. 2a). Using a k-mean
...
footprint index and longitudinal stream position (Fig. 3a, which were largely
unrelated to time series characteristics Fig. S5; see model predictions in
Fig. S6). Specifically, we found strong evidence that a higher degree of past
...
longitudinal stream position (i.e. PCA axis with high values associated with more downstream areas, see Methods), such as the legacy effects of past anthropogenic pressures on total
...
irrespective of the longitudinal stream position
(resp. CI 80%:$\beta\prime =$ `r rivhfthill` and `r rivhftturn`, Fig. 3a) -- a
result consistent between Jaccard and Simpson-based dissimilarity (Fig. S7). But
the effects of past anthropogenic pressures was attenuated in most downstream
areas when considering Jaccard based dissimilarity, indicating that those
changes implies more dominant species (Fig. S7).
...
recent anthropogenic pressures were associated with an increase in the proportion of non-native individuals
...
suggests that integrating fine scale local drivers may improve our ability to
predict local community changes.
...
species; this effect increasing in downstream sites.
...
an
increased of non-native species and lower species richness.
...
4% on average (comparison of the human footprint index between 1993 and 2009)
across the sites included in our study while it increased by 25% over the rivers
globally. Consequently, species richness and abundance increases as well as the
...
asmore tropical data are collected and become available.
...
`r perc_opa09` after 2009; Fig. S1). We chose the human footprint index developed
by Venter et al.[@venter_global_2016] because it goes farther back in time  than other related
human footprint index products (e.g. 1993 vs 2000 in Mu et al.[@mu_global_2022]) but note
that estimates in more recent years are highly correlated (Spearman's $\rho$ = `r s_mu_venter`
for 2009). In order to obtain interpretable coefficients of recent changes in
human footprint, we log-transformed the ratio
of human footprint with a base 2, such as values of -1 and 1 represented a
...
that we extracted from the HydroAtlas database at the stream segment
...
negative values in altitude) and standardising all the variables, i.e. centring
and scaling. We orthogonally rotated the two first principal components using
...
incorporating additional covariates into the models (eq. \@ref(eq:drivers)): the
...
anthropogenic pressure would indicate that faster changes were happening in
...
For instance, a positive interaction between time, recent changes in
anthropogenic pressures and longitudinal stream position would indicate that the
effects of recent changes in anthropogenic pressures were more important in
downstream areas. We included all the predictors as main effects
($\sum_{k=1}\beta_kX_k$) to capture the effects of the ecological drivers on the
spatial variation in average community composition metrics through time, except
when modelling the dissimilarity metrics (i.e. we only included the effects of
ecological drivers on the temporal trends) (i.e., the results in Fig. 3b). We
did so because dissimilarity metrics quantify the community change at a given
time point ($t = 0 \ldots N$) at a given site from the first year ($t = 0$) and
therefore bounded by 0 and 1 at each site; we thus did not expect average
differences in dissimilarity related to any other factors than time. We did not
include interactions among ecological drivers on the spatial part of the model
as we restricted the core of the analysis on the drivers of temporal rather than
spatial variation in freshwater fish community composition and tried to keep the
model as parsimonious as possible. We finally derived the comparison in community metrics
across space according to the ecological drivers by using the predictions of the
model at the baseline ($t = 0$). The predictions controlled for the values of
other predictors (such as longitudinal stream position and past anthropogenic
pressures) by setting them at their median values.
...
then computed marginal ($R^2_m$) and conditional ($R^2_c$) R-squared (Eq. \@ref(eq:r2)),
...
following recommendations to take into account the variability associated with
...
`r k["log_total_abundance"]` vs `r k["perc_exo_abun"]` and, `r k["log_chao_richness"]`, and `r k["perc_exo_sp"]`,
for resp. total and non-native abundance, and species richness, Fig. 1),
