...
characterized by significant changes in the dominant species, together with a
...
in response to human pressures from local to continental extents.
...
covariations among different community metrics, and identifying trajectories of community
...
and longitudinal stream position. Outcomes of this study further our
...
and site levels (see Methods for a detailed description of the models), we found
that communities have increased in both total abundance and species
...
pca_clust_cap <- paste0("**Covariation among the community temporal trends and
  ...
  limits display 1.5 interquartile range. (d) Cluster
...
metrics; the first two axes of the principal component analysis explained `r sum(var_exp[1:2])`%
of the total variability among fish communities (Fig. 2a-b). Temporal trends in
community composition (i.e. temporal dissimilarity and turnover) were positively
associated with each other as were temporal trends in total abundance and species richness;
however these two sets of trajectories appeared orthogonal to each other (Fig.
2b). Using a k-mean trimmed clustering method (see Methods), we further detected six distinct
...
community trajectories was broadly similar across the three main biogeographic realms
(Fig. 2d).
...
footprint index and longitudinal stream position (Fig. 3a, see model predictions
...
longitudinal stream position (i.e. PCA axis with high values associated with a
...
richness than the upstream-most sites (Fig. 3b). Noteworthy, a larger
...
over time could thus result from introduction of non-native species from ongoing
...
an
increased increased of non-native species and lower species richness.
...
most prevalent in tropical biodiversity hotspots[@venter_global_2016; @mu__2022], while our
...
the multidimensionality of community changes to the interplay between past and present environmental
...
communities. We showed that the timing of anthropogenic
...
sampling events in a given year, we selected the one that took place at the
...
`r tss$span` years ([25th quantile, 75th quantile]), the median completeness (i.e. number of annual samplings / time span) was
of `r tss$completeness` (see Fig. S1), and the median
...
scale[@linke_global_2019; @robinson_earthenv-dem90_2014]. We next performed a Principal Component Analysis over
...
variable describing the longitudinal stream position from upstream to
downstream. The correlation coefficients indicated little covariations among the
...
covariates into the models (eq. \@ref(eq:drivers)): the degree of past
...
year ($t = 0$) and therefore bounded by 0 and 1 at each site; we thus did
...
fitted versus observed values (Fig. S8). We visually inspected the PIT and
...
heavy tailed distribution compared to the other variables (Fig. 1, Kurtosis of the distributions:
`r k["perc_exo_abun"]`, `r k["perc_exo_sp"]`, and `r k["log_chao_richness"]`,
for resp. non-native abundance, non-native richness, and species richness),
which in turn exerted a disproportionate constraint on the analysis. In
addition, we performed a clustering analysis to identify distinct types of
...
outlying data trimmed ($\alpha$) and the number of clusters (Fig. S9). We
