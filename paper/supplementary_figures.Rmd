...
in the Nearctic realm (+5\\%, i.e. around 40 clusters over 897).") %>%
  str_replace_all("\\s+", " ")
if (knitr::is_latex_output()){
...
```{r}
std_tot_cap <- "**Effects of anthropogenic pressures and stream longitudinal position on (a)
...
streams**. PCA axes (Rotated Component, RC) were rotated using the Varimax algorithm
...
modelling as a composite variable intended to summarise the stream longitudinal position from upstream
to downstream." %>%
  str_replace_all("\\s+", " ")
...
observe very low correlations among the ecological drivers (stream longitudinal
  position, past
...
data_ref_cap <- "**References of the dataset included in the study**, arranged by numeric sampling size. Numeric Source ID correspond to the ID of the RivFishTIME database (Comte et al.,
...
```{r, eval=TRUE}
tab_quali_cap <- "**Site location, protocol and abundance unit occurrences
across sites, and data source for native species status** in term of species
occurrence." %>%
str_replace_all("\\s+", " ")
