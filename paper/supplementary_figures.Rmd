...
assigned to a cluster are not displayed (See supplementary map).", na_cluster, ".") %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
while non-native abundance, non-native abundance, dissimilarity and turnover are
expressed in proportion. No pressure, medium pressures, and highly degraded past
pressure levels correspond respectively to values of Human footprint index at
year 1993 of 2.5 (intact ecosystem), 16.8 (median value in our dataset), 45.6
(maximum value in the dataset). Recent pressure levels represent change in Human
footprint index between years 1993 and 2009 (see Methods for details). The
central lines display the mean, while the lower and upper lines display
the credible intervals at 95% of the posterior distribution."
```
...
(d) completeness of time series by site. In (a) and (b), the lines display the
year 1993 and 2009."
```
