...
time series, (c) time span, (d) completeness of the time series, (e) time series
of yearly sampling events**. Colours correspond to the biogeographic realms. In
(a), (b), and (e), the lines display the year 1993 and 2009. The years 1993 and
2009 correspond to the years of the human footprint index assessments, which
were used to quantify past and recent changes in anthropogenic pressures (See
  Methods)." %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
...
```{r base-span-cpl, fig.cap=cap, out.width = "80%", out.height = "95%", fig.show = "hide"}
...
timeseries_dist <- plot_grid(top, mid,
  plot_grid(
    tar_read(p_timeseries) +
      theme(
...
```{r}
tar_load(p_site_trends_ts_caracteristics2)
ggsave(
  filename = here("paper", "fig_paper", "p_site_trends_ts_caracteristics.png"),
  plot = p_site_trends_ts_caracteristics2,
  scale = 3.2,
  width = 80,
  height = 80 * 1.2,
  units = "mm",
...
ggsave(
  filename = here("paper", "fig_paper", "p_site_trends_ts_caracteristics.pdf"),
  plot = p_site_trends_ts_caracteristics2,
  scale = 3.2,
  width = 80,
  height = 80 * 1.2,
...
cov_ts_site_cap <- "**Covariations between temporal trends estimated at the site
level and the characteristics of the time series**. The year span is expressed
in number of years, completeness in proportion of years over the entire study
period and first year of sampling in year. The temporal trends were estimated
with time as sole fixed predictor (See Methods, eq. (3)). The black lines
display the regression lines fitted with a Generalized Additive Model (GAM) and
the gray area the confidence interval." %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")

if (knitr::is_latex_output()){
  cov_ts_site_cap <- str_replace(cov_ts_site_cap, bold_pattern, "\\\\textbf{\\2}")
}
```

```{r trends-site, fig.cap=cov_ts_site_cap, fig.asp = 1.1, out.width = "80%", fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "p_site_trends_ts_caracteristics.pdf"))
```
...
(BLUP) estimated at each site. The points depict the average posterior distribution
...
```
```{r trends-dist, fig.cap=trends_cap, fig.asp = 1.1, out.width = "80%", fig.show = "hide"}
...
pca_clust_cap <- paste0("**Covariation among the community temporal trends and
  characterization of community trajectories**. (a, b) PCA biplot of the
  community temporal trends and their cluster assignment where the sites are
  colored according to their cluster assignment. (c) Boxplots displaying the
  distribution of the temporal trends by cluster. The center of
  ...
  range beyond the bounds of the box. (d) Cluster frequencies across
  the three main biogeographic realms. The ellipses in (a-b) display the 95\\%
  intervals around the clusters assuming a Student’s t distribution. The
  clusters were named according to their main characteristics. Contrary to Fig.
  2 (Main text), we included the sites whose cluster affiliation was uncertain
  ('NA', black color, ", na_cluster, ".", " NA clusters are well distributed
  over the PCA (a-b), which is confirmed by the distribution of community metric
  temporal trends (c). We observe a few more NAs in the Nearctic realm (+5\\%,
    i.e. around 40 sites over 897).") %>%
str_replace_all("\\s+", " ")
...
```{r cl-map, fig.cap=pca_clust_cap, fig.asp = 1.1, out.width = "80%", fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "clust_figure_na.pdf"))
```

```{r}
tar_load(p_drivers_ts_caracteristics)
ggsave(
  filename = "p_drivers_ts_caracteristic.pdf",
  plot = p_drivers_ts_caracteristics,
  path = here("paper", "fig_paper"),
  scale = 2.2,
  width = 80,
  height = 80 * 1.0,
...
```{r}
drivers_ts_cap <- paste0("**Covariations between ecological drivers used in the
  hierarchical Bayesian model** (longitudinal stream position, past and recent
    changes in anthropogenic pressures) and time series characteristics (time
    span in number of years and beginning of the time series in year). The blue
  lines display the regression lines fitted with a Generalized Additive Model
  (GAM) and the gray area the confidence interval. The ecological drivers were
  largely unrelated to the characteristics of the time series
  (N = ",fnsite," sites).") %>%
  str_replace_all("\\s+", " ")

if (knitr::is_latex_output()) {
 drivers_ts_cap <- str_replace(drivers_ts_cap, bold_pattern, "\\\\textbf{\\2}")
}
```

```{r drivers-ts, fig.cap=drivers_ts_cap, fig.asp = 1.1, out.width = "80%", fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "p_drivers_ts_caracteristic.pdf"))
```
...
richness, dissimilarity and turnover are expressed in proportion. No pressure,
...
```{r, out.width = "70%", out.height = "85%", fig.cap = pred_hft_cap, fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "p_pred_hft.pdf"))
```

```{r}
std_tot_cap <- paste0("**Effects of anthropogenic pressures and longitudinal stream position on (a)
...
changes in the identity of dominant species (see
  Methods). The points depict the average posterior distribution. Large, medium and thin bars depict the Bayesian credible intervals
at 80, 90 and 95\\%, respectively. Please note the broken abscissa scale in (a).
N = ", fnop, " sampling events across ", fnsite," sites.") %>%
  str_replace_all("\\s+", " ")
...
```{r std-tot, fig.width = 8, fig.asp = .5, fig.cap=std_tot_cap, fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "p_effect_supp.pdf"))
```
...
```{r}
cap_pca_riv <- "**Rotated principal component analysis (PCA) over the physical
and hydrological characteristics of the sites**. PCA axes (Rotated Component,
  RC) were rotated using the varimax algorithm (See Methods). The first axis is
related to the average discharge, Strahler order and the distance from source,
and was subsequently used in the statistical modelling as a composite variable
intended to summarise the longitudinal stream position from upstream to
downstream." %>% str_replace_all("\\s+", " ")
...
ggsave(
  filename = "p_pca_riv_str.pdf",
  plot = p_pca_riv_str +
    theme(plot.margin = margin(t = 5, r = 8, b = 2, l = 2, unit = "pt")),
  path = here("paper", "fig_paper"),
  scale = 1.4,
  width = 80,
  height = 80 * .90,
...
```{r pca-riv-str, fig.width = 6, fig.asp = 1, fig.cap=cap_pca_riv, fig.show = "hide"}
knitr::include_graphics(here("paper", "fig_paper", "p_pca_riv_str.pdf"))
```
...
RiverAtlas and are meant to illustrate the covariations of various
human pressures with the human footprint index. We show the Spearman correlation with the
...
position, past pressures and recent pressures). Past pressures (human footprint
index for year 1993) are positively correlated with population density, urban
extent and cropland extent, but are negatively correlated with
forest extent." %>% str_replace_all("\\s+", " ")
...
composition (Fig. 2, main text). The center of the box depicts the median while
the bounds depicts the 25\\% and 75\\% percentile. The whiskers depict the extreme
values within 1.5 interquartile range beyond the bounds of the box." %>%
  str_replace_all("\\s+", " ")
...
tab_rand_ok <- tab_rand2 %>%
  pivot_wider(names_from = "Spatial scale", values_from = "Temporal trends s.d.") %>%
  select(1, 3, 2) %>%
  kbl(booktabs = T,
    format = "latex",#ifelse(knitr::is_latex_output(), "latex", "html"),
    label = "",
...
kableExtra::save_kable(
  ...
  self_contained = TRUE,
  density = 600,
  file = here("paper", "fig_paper", "tab_rand_extended_data_table1.jpg"),
  keep_tex = TRUE
)
tab_rand_ok
```
