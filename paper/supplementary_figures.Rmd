...
```{r}
tar_load(c(pca_clust, site_cl_rm, site_no_drivers_inla_tot, site_cl_na))
...
```{r}
pca_total_cap <- "**PCA biplot on the temporal trends of the community metrics
 at each site, and their cluster assignment**. See legend of the Fig. 1 in main
 text for details. To the difference with the Fig. 1, we added Jaccard
 dissimilarity index and its partitions in two sets of complementary indices,
 respectively nestedness/turnover, and appearance/disappearance." %>%
  str_remove_all("\n|\r") %>%
  str_replace_all("  ", " ")
```
...
```{r}
raw_chao_caption <- "Comparison of the model estimates between Chao-corrected
species richness and raw measures of species richness. The results are highly
similar."
```

```{r raw-chao, fig.cap =raw_chao_caption}
...
col = c(pal_variable[names(pal_variable) == get_var_replacement_vulgarisation()["log_chao_richness"]], "red")
names(col) <- get_var_replacement_vulgarisation()[c("log_chao_richness", "log_species_nb")]

base_width <- .2
increment <- .25

tar_load(gaussian_inla_std_effects)
rbind(
  ...
  mutate(
    ...
    response = get_var_replacement_vulgarisation()[response],
    response = factor(response, levels = rev(names(col)))
  ) %>%
  filter(response %in% names(col)) %>%
...
raw species richness, added to their respective distribution. The black line
