...
" Contrary to the Fig. 2 (Main text), we included the NA (black color) clusters",
na_cluster, ".",
...
medium pressures, and highly degraded past pressure levels correspond
respectively to values of Human footprint index at year 1993 of 2.5 (intact
ecosystem), 16.8 (median value in our dataset), 45.6 (maximum value in the
dataset). Recent pressure levels represent change in Human footprint index
between years 1993 and 2009 (see Methods for details). The central lines display
...
concerned abundant species and thus changes in the identity of dominant species (Hillebrand et .al. 2018, see Methods)."
```
...
streams. PCA axis (Rotated Component, RC) were rotated using Varimax algorithm
(See Methods). The first axis is related the average discharge, the Strahler
order and the distance from source. This first axis was used in the statistical
...
in the hierarchical Bayesian model. Land-use data and population density were
extracted from HydroAtlas, which uses different data than the Human Footprint
Index (See Methods). The extents of forest, pasture, and cropland at year 2000,
urban extent at year 2015 and population density was provided for
year 2010 (See Linke et al., 2019 for details). We observe very low correlation
among the ecological drivers (Stream gradient, past pressures and recent
pressures). Past pressures (human footprint index at year 1993) are positively
correlated to population density, urban extent and cropland extent, but are
negatively correlated with Forest extent." %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
obs_fit_cap <- paste0("Fitted values of the models vs observed values (N = ",N,"). The black line
...
The Pearson's correlation coefficient is displayed in each panel. We see that
the model predicted the data reasonnably well, but it tends
to overestimate low values and underestimate really high values especially for very high proportion of non-native species and very high dissimilarity.") %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
multidimentional space). In the results presented in the main text, we
removed 5\\% of the data. We see that the goodness of fit of the clustering does not
increase a lot from 4 clusters and does not increase from 7 clusters." %>%
str_replace_all("  ", " ") %>%
str_replace_all("\n|\r", " ")
```
...
we observe the disappearance of the clusters related to changes in community
...
the start of sampling or the log of the former variable added to one (See
Methods). Most community metrics are best modelled with the log of time
variable." %>%
str_replace_all("\n|\r", " ")
```
