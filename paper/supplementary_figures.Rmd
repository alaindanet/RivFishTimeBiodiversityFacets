...
```{r}
# get the final site and op used in modelling
# i.e. remove the NA due to missing environmental variables
filtered_dataset <- map(filtered_dataset, function(x, stat_data) {
...
filtered_op_protocol <- filtered_op_protocol %>%
  filter(op_id %in% filtered_dataset$abun_rich$op_id)
```
...
```{r}
year_op_cap <- "Distribution of year of samplings. Years 1993 and 2009 are highlighted because they correspond to the two human footprint measurements."
```

```{r y-op, fig.cap=year_op_cap}
nb_op9309 <- sum(
  filtered_op_protocol$year >= 1993 &
  filtered_op_protocol$year <= 2009
)
perc_op9309 <- scales::percent(nb_op9309 / nrow(filtered_op_protocol))
