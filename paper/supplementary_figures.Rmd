...
```{r tab-exo-source-ck, results = "hide"}
exotic_source_summary %>%
  kbl(booktabs = T,
...
txt <- tibble(
 ...
 y = c(8500, 8500, 6000, 8500, 6500, 6000),
 label = c(
   paste0(
...
```{r base-span-cpl, fig.cap=cap, fig.asp = 2}
...
rbind(
  ...
  kbl(booktabs = T,
    label = "tab-resp",
    caption = paste0("Summary descriptors of response and environmental
      variables. Main text
    names display the name used in the legend of the figures in main text. The
    community metrics that do not have main text name were not used in the
...
```{r std-tot, fig.asp = 2.5, fig.cap=std_tot_cap}
figS1
```
...
```{r pca-tot, fig.cap = pca_total_cap, fig.asp = 1.2}
...
# exclude riv
pred_to_keep <- names(pred_gaussian_inla_tot$pred_plot[[1]])[!str_detect(names(pred_gaussian_inla_tot$pred_plot[[i]]), "riv")]
...
```{r, fig.asp = 3, fig.cap=trends_cap}
