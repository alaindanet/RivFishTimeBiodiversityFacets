...
```{r}
get_ci_fig <- function(response, mean, low, high, ...){
  case_when(
    response == "Abundance (total, %)" ~ p_ci(
      x = c("mean" = mean, "low" = low, "high" = high), r = 1),
    response == "Richness (%)" ~ p_ci(
      c(mean = mean, low = low, high = high), r = 1),
    response == "Non-native abundance (proportion)" ~ p_ci(
      c(mean = mean, low = low, high = high), r = 4, p = FALSE),
    response == "Non-native richness (proportion)" ~ p_ci(
      c(mean = mean, low = low, high = high), r = 3, p = FALSE),
    response == "Turnover (%)" ~ p_ci(
      c(mean = mean, low = low, high = high), r = 1, p = TRUE),
    response == "Dissimilarity (%)" ~ p_ci(
      c(mean = mean, low = low, high = high), r = 1, p = TRUE),
    TRUE ~ "null"
  )
}
...
ci_label_data %<>%
  ...
  label = pmap_chr(list(response, mean, low, high), get_ci_fig) %>%
    str_replace(., "\\[\\s", "[") %>%
    str_trim(., side = "both"),
  label_y_pos = error_bar_pos,
  label_x_pos = case_when(
      response == "Abundance (total, %)" ~  high + 2/100 * high,
      response == "Richness (%)" ~ high + 5/100 * high,
      response == "Non-native abundance (proportion)" ~ high + 8 * high,
      response == "Non-native richness (proportion)" ~ high + .8 * high,
      response == "Turnover (%)" ~ high + 1/100 * high,
      response == "Dissimilarity (%)" ~ high + 1/100 * high,
      TRUE ~ 1000 * .10
    )
  )
```
...
p_trends_site2 <-
  ...
  geom_text(
    data = ci_label_data %>%
      filter(ci_level == "95%") %>%
      arrange(desc(width_bar)),
    aes(y = label_y_pos, x = label_x_pos,
      label = label),
    size = 3,
    hjust = -.03, vjust = .4,
    show.legend = FALSE
...
```{r}
tar_load(c(site_no_drivers_inla, site_trends_dec))
```
