...
trends_10y %<>%
  ...
  (response == "perc_exo_sp" &  ci_level == "80%") |
  (response == "perc_exo_abun" &  ci_level == "90%")
  ) %>%
  # Format CI
  mutate(
    ci_label = pmap_chr(list(response, ci_level, low, high, mean),
      function(response, ci_level, low, high, mean) {
        if (response %in% c("log_total_abundance", "log_chao_richness")) {
        ci <- p_ci(
          c(mean = mean, low = low, high = high),
          r = 1, p = TRUE
        )
        ci <- str_replace(ci, "\\[ ", "\\[")
        } else if (response %in% c(
            "hillebrand_dis_scaled",
            "jaccard_dis_scaled",
            "nestedness_scaled",
            "turnover_scaled")) {
        ...
        } else if (response == "perc_exo_sp") {
          ci <- p_ci(
            c(mean = mean, low = low, high = high),
            r = 3, p = FALSE
          )
        } else if (response == "perc_exo_abun") {
          ci <- p_ci(
            c(mean = mean, low = low, high = high),
            r = 4, p = FALSE
          )
        }
        paste0("CI", ci_level, ": ", ci)
      }
    )
  )
...
perc_pal_variable <- setNames(pal_variable,
      c("Abundance (total, %)", "Richness (%)",
        names(pal_variable)[3:6]
      ))
...
ci_label_data <- trends_10y %>%
  filter(response %in% restricted_dimension) %>%
  mutate(
    response = get_var_replacement_vulgarisation()[response],
    response = str_replace_all(response,
      c("Abundance \\(total\\)" = "Abundance (total, %)",
        "Richness" = "Richness (%)"
      )
      ),
    response_f = factor(response, levels = names(perc_pal_variable)),
...
p_trends_site <- tps_trends_10y %>%
  ...
  facet_wrap(~response_f, scales = "free", ncol = 2, strip.position = "top") +
  theme_cowplot(8) +
  theme(
...
```{r}
TODO: Jaccard and nestedness in supplementary
```
...
fig1 <- plot_grid(
  print(p2),
  ggdraw(p3 + theme(legend.position = "none"), xlim = c(-.02, 1.015), ylim = c(-.01, 1.01)),
  print(p1),
  leg_dis,
...
p_pca_cl_bt <- target_bp_cl_dist(
  ...
  fct_var_lvl = names(pal_variable)[c(3:4, 1:2)],
  dodge = .85
  ) +
  scale_color_viridis(discrete = TRUE) +
...
fig2 <- plot_grid(
  p_pca_cl_top,
  plot_grid(p_pca_cl_bt, leg_cl, rel_widths = c(1, .25), labels = c(letters[3], NULL)),
  nrow = 2,
  rel_heights = c(1, .8)
)
#fig2
...
leg_cl_right <- get_legend(
  p_pca_cl_ell[[1]] +
    ...
    guides(col = guide_legend(
        nrow = 1, override.aes = list(size = 10)
        ))
    )
bottom <- plot_grid(p_tot_tab, leg_cl_right, ncol = 1, rel_heights = c(1, .11),
  labels = c("iv)", ""))
#fig3_alt <- plot_grid(top, bottom, nrow = 2, rel_heights = c(1, .40))

inset_tot <- plot_grid(plotlist = append(
...
fig3_alt <- plot_grid(
  inset_tot,
  bottom, nrow = 2, rel_heights = c(1, .70))
ggsave(
  filename = "fig3_alt.pdf",
  plot = fig3_alt,
  path = here("paper", "fig_paper"),
  scale = 5.0,
  width = 88,
...
basin_map <- filtered_basinatlas_modelling %>%
  ...
  mutate(basin_name = str_replace_all(main_bas, get_rev_vec_name_val(c(pal_basin, nea_basin, aus_basin))),
    label = str_replace_all(basin_name, get_rev_vec_name_val(all_basin)),
    label2 = str_replace_all(label, c("l" = "l     ", g = "   g", d = " d", "k" = "k   ")),
  )
