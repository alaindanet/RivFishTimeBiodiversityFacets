...
var_labels <- map_chr(names(pal_variable),
  ~paste0(
    "'", .x,"'",
    "~(R^2 == ",
    r2_obs_fit[r2_obs_fit$response == .x, ]$gelman_r2,
    ")")
)
leg_dis <- get_legend(
  p3 +
...
fig1_cap <- paste0(
  ...
  dissimilarity metrics (See methods). Please note the broken abscissa scale in
  the framed panels. N = ", fnop, " sampling events over ", fnsite, " sites.") %>%
  str_remove_all("\n|\r") %>%
  str_replace_all("  ", " ")
```
