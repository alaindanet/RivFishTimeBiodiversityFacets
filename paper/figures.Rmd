...
fig1_cap <- paste0(
  ...
  richness, the proportion of non-native richness and of non-native abundance.
  ...
  80, 90 and 95%. Main effects but time were not included for the site
  dissimilarity metrics (See methods). R-Squared conditional to the hierarchical
  structure of the model are displayed in the legend. Please note the broken
...
```{r, results = "hide"}
summary(pca_clust$normal)
summary(pca_clust$rotated)

summary(pca_clust2$normal)
summary(pca_clust2$rotated)
```


```{r}
debugonce(plot_pca_clust)
...
pca_clust_cap <- paste0("**PCA biplot on the temporal trends of the
  ...
  BLUP (see methods). PCA with two and three
  principal components explained respectively ",
  paste0(pca2_var_expl, " and ", pca3_var_expl), " of the variance. The temporal
  trends of community metrics were clustered using the k-means method while
  excluding 5% of the most outlying sites and sites with a doubtful assignement
  to a cluster were removed ", na_cluster, ".  The sites are colored by their
  cluster belongings. The ellipses represent the 95% interval around the
  clusters assuming a Student's t distribution. The clusters were named
  according to their unique characteristics. Scaled temporal trends are
...
fig2 <- plot_grid(
  p_pca_cl_top,
  plot_grid(p_pca_cl_bt, leg_cl, rel_widths = c(.75, .25)),
  nrow = 2,
  rel_heights = c(1, .8)
)
```
...
fig3 <- plot_grid(
  ...
  plot_grid(fig3_bottom_left3, leg_cl, NULL, nrow = 1, rel_widths = c(1, .2, .7)),
  ncol = 1
  )
ggsave(
  filename = "fig3.pdf",
...
trends**. Left panel: Spatial distribution of the the clusters with insets of
...
most sampled hydrographic basins, corresponding to the inset of the left panel.
The number of sites and the country are displayed between parenthesis."
```
