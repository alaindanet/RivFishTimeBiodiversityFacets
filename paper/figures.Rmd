...
```{r map-cl3, fig.width = 4.75, fig.asp = .42, fig.show = "hide", eval=FALSE}
layout <- "
AAAAAH
AAAAAH
BDEFGC
...
log10(ratio_width_to_a + 1) / max(log10(ratio_width_to_a + 1))
...
map_cl_fin <- main_map_rect_basin + p_tot_tab +
  inset_basin[[1]] + inset_basin[[2]] + inset_basin[[3]] +
  inset_basin[[4]] + inset_basin[[5]] + inset_basin[[6]] +
  inset_basin[[7]] + inset_basin[[8]] + inset_basin[[9]] +
  leg_cl_right +
  plot_layout(design = layout)
map_cl_fin
ggsave(
...
tab_rand <- gaussian_inla_rand %>%
  ...
  select(
    ...
    Term = term,
    `Standard deviation mean [95% CI]` = ci
  )
...
tab_rand2 <- tab_rand %>%
  left_join(
    tu %>%
      select(
        `Response variable` = response,
        `Marg. R2 mean [95% CI]` = r2_mvp_marg95hpdci,
        `Cond. R2 mean [95% CI]` = r2_mvp_cond95hpdci
        ),
...
```{r}
tab_rand_cap <- "Marginal and conditional R-squared of the models and the standard deviation estimated for the random effect and the error terms. CI: Credible Interval computed using Highest Posterior Density method."
tab_rand2 %>%
  kbl(booktabs = T,
    label = "tab-rand",
