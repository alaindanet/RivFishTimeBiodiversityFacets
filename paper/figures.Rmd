...
p_trends_site <- tps_trends_y %>%
  ...
  labs(x = "Temporal trends per year", y = "Number of sites") +
...
```{r}
tar_load(c(site_cl_na))
axis_l <- list(
  c("CS1", "CS2"),
  c("CS3", "CS4")
)
table_to_vec(table(site_cl_na$cl))
sum(is.na(site_cl_na$cl))
...
```{r}
tar_load(c(site_cl_na, filtered_dataset_modelling))
loc <- filtered_dataset_modelling$location %>%
  mutate(ecoregion = str_replace(ecoregion, "Neartic", "Nearctic")) %>%
  left_join(filtered_dataset_modelling$site_quali, by = "siteid") %>%
  st_as_sf(coords = c("longitude", "latitude"),
