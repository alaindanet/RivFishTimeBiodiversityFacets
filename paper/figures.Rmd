...
restricted_dimension <- c("log_total_abundance", "log_chao_richness",
  "perc_exo_sp", "perc_exo_abun", "turnover_scaled", "hillebrand_dis_scaled")
```
...
hist(site_trends_dec$log_total_abundance)
```
...
```{r} 
pca_clust_map <- paste0("**Covariation among the temporal trends in biodiversity metrics
...
fig3_alt <- plot_grid(inset_tot, bottom, nrow = 2, rel_heights = c(1, .70))
ggsave(
...
```{r}
tab_rand_cap <- "R-squared of the models Conditional mean R2 of the models and the mean standard deviation estimated for the random effect and the error terms. [95% CI]: Credible Interval computed using Highest Posterior Density method."
tab_rand2 %>%
  kbl(booktabs = T,
    label = "tab-rand",
