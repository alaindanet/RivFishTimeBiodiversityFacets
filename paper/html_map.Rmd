...
hft <- analysis_dataset %>%
  st_drop_geometry() %>%
  mutate(
    hft_ix_c9309_perc_diff = 100 * (hft_ix_c09 - hft_ix_c93) / hft_ix_c93,
    hft_ix_c9309_perc_diff = round(hft_ix_c9309_perc_diff)
    ) %>%
  select(siteid, hft_ix_c93, hft_ix_c9309_perc_diff) %>%
  distinct(siteid, hft_ix_c93, hft_ix_c9309_perc_diff) %>%
  na.omit
...
trends_cl_loc <- site_cl_na_loc %>%
  ...
  left_join(hft) %>%
  mutate(
    cl_f = factor(cl, levels = cluster_names)
    ) %>%
  rename(
    `Cluster assignement` = cl_f,
    `Site id` = siteid,
    `Past pressures` = hft_ix_c93,
    `Recent pressures (changes in %)` = hft_ix_c9309_perc_diff
    ) %>%
  select(all_of(c(
        "Cluster assignement", names(restricted_dimension),
        "Site id", "Past pressures", "Recent pressures (changes in %)")
    )
  )
```
...
mapview(trends_cl_loc %>%
  ...
  at = c(-10, 0, 10, 25, 50, 75, 100)) +
mapview(trends_cl_loc,
  zcol = c("Past pressures"),
  layer.name = c("Past pressures"),
  label = c("Past pressures"),
  homebutton = FALSE,
  at = c(0, 4, 10, 25, 50)) +
mapview(trends_cl_loc,
  zcol = c("Recent pressures (changes in %)"),
  layer.name = c("Recent pressures (changes in %)"),
  label = c("Recent pressures (changes in %)"),
  homebutton = FALSE,
  at = c(-50, -25, 0, 25, 100, 200)
)
```
```{r}
mapshot(m,
  url = here("paper", "fig_paper", "SupplementaryMap", "SupplementaryMap1.html"),
  selfcontained = FALSE
)
```
