...
```{r}
#  
tar_load(c(wt, formated_wt))
write_csv(wt, file = here("data", "filtered_water_temperature.csv"))
write_csv(formated_wt, file = here("data", "no_filtered_water_temperature.csv"))
```
...
na %>%
  group_by(year) %>%
  summarise(na_sum = sum(n_na)) %>%
  ggplot(aes(x = year, y = na_sum)) +
  geom_line()
```
