...
ex <- vegdist(neutral_com$sim[[1]], method = "jaccard")

```

```{r}
neutral_com$temporal_jaccard <- map(
  neutral_com$sim,
...
ti <- neutral_com %>%
  ungroup() %>%
  filter(
    immigration == max(immigration),
    death_rate == min(death_rate),
    abun == max(abun)
  )
...
get_vegdist_temporal_turnover_c(
  mat = as.matrix(ti$sim[[3]][250:500, ]),
  method = "jaccard",
  return_tibble = TRUE,
  drop_first_year = TRUE
)
