...
```{r}
ti <- matrix(rep(NA, 10), ncol = 2, dimnames = list(seq(1,5), letters[1:2]))
to_add <- list(c = rep(1,5), d = rep(2, 5))
tu <- matrix(
  unlist(to_add), ncol = length(to_add), byrow = FALSE,
  dimnames = list(NULL, names(to_add)))
cbind(ti, tu)
```
...
for(i in seq(10)) {
  x[i] <- mean(vegdist(com_mat_basin$mat[[i]], method = "jaccard"))
}
test <- com_mat_basin %>%
filter(main_bas == unique(main_bas)[1])

# Keep site present
common_site <- Reduce(intersect, map(test$mat, row.names))
#
site_nb_sampling <- map(test$mat, row.names) %>%
  unlist() %>%
  table
site_to_keep <- names(site_nb_sampling)[site_nb_sampling >= 10]

# Site sampled each year and for 10 years at least
common_site <- Reduce(intersect,
  map(test$mat, ~row.names(.x)[row.names(.x) %in% site_to_keep]
  )
)
```
```{r}
...
tee <- te %>%
  mutate(
    site_to_keep = list(
      select_rownames_in_matrix_list(data[["mat"]], min_nb_sampled = 10)
      ),
