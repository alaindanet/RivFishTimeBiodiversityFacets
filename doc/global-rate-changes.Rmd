...
tar_load(c(gaussian_inla_effects))
gaussian_inla_effects %>%
  filter(
...
gaussian_inla_effects %>%
  filter(
    response == "log_total_abundance",
    ci_level == "level:0.80", term == "log1_year_nb"
  )
gaussian_inla_no_drivers_effects %<>%
  filter(
    response %in% c("log_chao_richness", "log_total_abundance",
      "hillebrand_dis_scaled"),
    ci_level == "level:0.95", term == "log1_year_nb"
  )
```

# Temporal trends without drivers
...
```{r}
r_abun10 <- round(log_beta_to_perc_rate(abun_coef_vector) * log(10 + 1), 2)
```
...
```{r}
r_rich10 <- round(log_beta_to_perc_rate(rich_coef_vector) * log(10 + 1), 2)
```
...
# Temporal trends with drivers

With the ecological drivers, the global effect of time corresponds to a
situation where all the effects are equal 0, meaning that the Human Footprint
variables are  
equals to 0 (meaning wilderness sites with no recent changes in anthropogenic
pressure). But also that the value of the PCA axis related to the stream
gradient is 0.

## 

```{r}
get_effect_from_resp <- function(effect_obj = NULL, resp = NULL, term = "log1_year_nb", ci_lvl = "95") {
    effect_obj[
      effect_obj$response == "perc_exo_sp" &
        effect_obj$ci_level == paste0("level:0.", ci_lvl) &
        effect_obj$term == term,
      c("low", "high", "mean")
      ] %>%
        pivot_longer(everything()) %>%
        deframe()
...
get_effect_from_resp(
  effect_obj = gaussian_inla_exo_no_drivers_effects, resp =
  "perc_exo_sp",
  ci_lvl = "95"
) * log(10 + 1)
...
gaussian_inla_exo_no_drivers_effects %>%
  ...
  mutate(
    mean = map2_dbl(response, mean,
      ~compute_trends_meaningful_units(x = .y, resp = .x, time = log(10 + 1))),
