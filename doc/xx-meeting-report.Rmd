...
```{r}
# does not work for now because I estimated several times the basin random effect on
# intercept, normally its fixed
#x <- parameters(
#  gaussian_jaccard_tmb$mod[[6]],
#  group_level = TRUE,
#  effects = "random"
#)
...
pred_year_riv_str_fe <- gaussian_jaccard_tmb  %>%
  ...
  select(-mod)
plot_grid(plotlist = pred_year_riv_str_fe$pred_plot)
```
```{r}
pred_log1_year_riv_str_fe <- gaussian_jaccard_tmb  %>%
  ...
  select(-mod)
plot_grid(plotlist = pred_log1_year_riv_str_fe$pred_plot)
```
...
pred_year_hft_fe <- gaussian_jaccard_tmb  %>%
  ...
  select(-mod)
plot_grid(plotlist = pred_year_hft_fe$pred_plot)
```
```{r}
pred_log1_year_hft_fe <- gaussian_jaccard_tmb  %>%
  ...
  select(-mod)
plot_grid(plotlist = pred_log1_year_hft_fe$pred_plot)
```
