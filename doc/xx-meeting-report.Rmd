...
tar_load(c(var_analysis, tps_var))
tar_load(c(gaussian_comp_std, gaussian_int_env_comp_std, gaussian_re_self_c))

var_to_keep <- c(
  "jaccard_dis_scaled", "turnover_scaled",
  "nestedness_scaled", "hillebrand_dis_scaled",
  "appearance_scaled", "disappearance_scaled"
  "log_chao_richness", "log_total_abundance"
)

tar_load(gaussian_re_self_c)
...
@chao_coverage-based_2014. The Chao index correct the species richness for the sampling coverage. The sampling coverage of each sampling is evaluated by the total number of individual and the number of singleton and (double) species. All the sampling events were set at the same coverage of .80,
interpolation and rarefaction for respectively low and high coverage sampling. Chao index was logged to express the temporal trends of species richness of percentage variation. The temporal trends estimated from uncorrected species richness gave the same results (Figure SXX).
...
highly non linear (like a Michaelis-Menten), we logged the year to linearize the
relationship. AIC of the models with logged year were up to twice lower (Table
SXX).
...
the sensivity of our results to modelling choice. Values of
...
At the difference with dissimilarity index, we modelled species richness with the intercept and the main effect of ecological drivers. We modeled species richness with Chao index. 
...
CPUE, and Leslie index representing respectively XX% of the site. To account for
this hererogeneity, we included the unit of measurement as a main affect explaining total abundance but also
as an interaction with the year effect, in a similar way than previous studies [@klink_meta-analysis_2020].
Similar to species richness modelling, we modelled the log of total abundances. 
...
```{r cl-curv, fig.cap=kmeans_opt_nb_gr_sil_cap}
...
centered before clustering.  According to the cluster curver, the optimal
number of clusters was four (Fig. \@ref(fig:cl-sil)). We used 25 starting
...
filter(mod_comp_std_df, is.na(CI_low))
mod_comp_std_df %>%
  ggplot(
...
- Summary of results (Figure \@ref(fig:mod-slp-std)):
  - Temporal trends:
    - All the dissimiliraty indices increased in time :
      - Jaccard, abundance dissimiliraty and appearance have the higher temporal trends
        ($r_\delta$ around 0.37), followed by disappearance, nestedness and
        turnover ($r_\delta$ around 0.25)
    - Species richness and total abundance has a tiny increase around 0.025 (ten time less) and
      evenness did not change through time in average
  - River structure (upstream -> downstream, called hereafter stream gradient):
    - Species richness was higher toward downstream
    - But had no effect on total abundance
  ...
  - Interaction between temporal trends and stream gradient (year nb riv str rc1 effect):
    - Trends of total abundance and species richness were higher downstream than
      upstream
    - Turnover (i.e. change in species identity) was higher downstream than
      upstream but we found the opposite for Nestedness, which was lower downstream than
      upstream. Overall Jaccard was not affected by stream gradient (logical:
      knowing that turnover = 1 - nestedness; if jaccard is not affected by
      stream gradient, turnover and nestedness display necesseraly antagonistic effects).
    - Temporal trends of Appearance and Disappearance were not affected by
      stream gradient 
    - The trends of abundance dissimiliraty index was higher downstream then
      upstream but not the one base on presence / absence. 
  - Interaction between temporal trends and human footprint difference (year nb
     effect):
     - temporal change in Human footprint had a negative effect on trends of species richness but not
       on total abundance trends 
    - Increase Turnover 
    - Increase dissimiliraty of dominant species 
...
- Cluster analysis: 
  ...
  - Cluster 2: decrease abundance + medium dissimilarity 
  - Cluster 3: increase abundance + medium dissimilarity + low richness change 
  - Cluster 4: increase abundance + medium dissimilarity + increase richness +
    high nestedness 
  - Cluster 5: medium increase abundance + high dissimilarity +  medium increase richness +
    high turnover 
  - Cluster 6: No abundance change + medium dissimilarity + decrease richness
