...
```{r}
tar_load(c(gaussian_jaccard_tmb, gaussian_rich_tmb))
...
mod_tmb_comp_std %>%
  ...
  ggplot(
    aes(y = Parameter, x = Std_Coefficient, color = response, xmin = CI_low, xmax = CI_high),
    ) +
...
#  effects = "random"
#)
random_effect <- binded_gaussian_tmb %>%
  filter(intercept == 1, year_var == "year_nb")  %>%
  mutate(random_site = map(mod,
      ~try(get_random_effect_glmmTMB(
        .x, effect = "siteid:main_bas")
      ))) %>%
select(-mod)

site_temporal_trends <- random_effect %>%
  unnest(random_site) %>%
