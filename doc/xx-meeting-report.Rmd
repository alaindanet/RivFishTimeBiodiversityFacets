...
tar_load(c(binded_gaussian, gaussian_comp_std, gaussian_re_self_c))
...
gaussian_tmb <- binded_gaussian %>%
  filter(response %in% var_to_keep)
...
At each site, all the dissimilarity indices had a value of 0 the first year. Then we constrained the intercept to 0 and kept only random effect on the slope. Models comparison shows that the results are qualitatively the same.
...
mod_tmb_comp_std_df <- gaussian_comp_std %>%
  as_tibble() %>%
