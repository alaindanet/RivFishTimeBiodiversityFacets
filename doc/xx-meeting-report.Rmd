...
```{r mod-slp-std, fig.cap=mod_slp_std_cap, fig.width=10, fig.height=10}
...
leg <- get_legend({p + theme(legend.direction = "horizontal")})
p2 <- p +
  theme(
  ...
  if (add_variable) {
    ...
    p <- p +
      ...
      ggrepel::geom_label_repel(data = pca_label,
        ...
        box.padding = .1, label.padding = .35
      )
  }
  return(p)
}
...
```{r mod-tps, fig.cap=mod_slp_std_cap, fig.height=16}
...
1. Effect of interaction between pressure on temporal trends 
  1. Short-term pressure on the stream gradient:
    ...
    - Antagonistic effects on turnover and appearance, i.e. short-term changes decrease the effect on stream gradient 
  1. Long-term pressure on the stream gradient: 
    - Mainly very little effect
    - Antagonistic effects on abundance based dissimilarity, on turnover: i.e. long-term changes decrease the effect on stream gradient on turnover and abundance-based dissimilarity
...
```{r}
pca_clust_cap <- "(A) PCA biplot on site temporal trends (model without drivers). The sites are colored by their cluster belongings. (B) Correlation among temporal trends."
```
