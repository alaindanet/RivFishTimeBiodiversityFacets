...
```
```{r}
pred_perc_exo_sp <- pred_gaussian_inla_exo %>%
  filter(response == "perc_exo_sp") %>%
  pull(pred) %>%
  .[[1]]
ti <- pred_gaussian_inla_exo %>%
  mutate(pred_plot = map2(pred,response, ~ get_pred_list_plot(.x, get_var_replacement()[.y])))
```
