...
tar_load(c( modelling_data))
tar_load(c(abun_var, var_jaccard, rich_var))
tar_load(c(binded_gaussian_tmb_no_evt_re))
```
...
```{r}
dis <- model_to_compare[model_to_compare$response == "disappearance_scaled", ]$mod[[1]]
dis2 <- update(dis,
  formula = disappearance_scaled ~
    log1_year_nb * riv_str_rc1 +
    log1_year_nb * hft_ix_c9309_diff_scaled +
    (0 + log1_year_nb | main_bas/siteid),
  REML = TRUE
...
```{r}
comp_intercept <- compare_models(zc, zc0, zc0_no_main, effects = "fixed")
comp_intercept %>%
  as_tibble() %>%
...
- test with nestedness:

```{r}
ne <- glmmTMB(formula = nestedness_scaled ~
    log1_year_nb * riv_str_rc1 +
    log1_year_nb * hft_ix_c9309_diff_scaled +
    (0 + log1_year_nb | main_bas/siteid),
  data = modelling_data 
...
```{r}
comp_intercept <- compare_models(ne, ne0, ne0_no_main, effects = "fixed")
comp_intercept %>%
  as_tibble() %>%
  select(Parameter, starts_with("CI_"), starts_with("Coefficient")) %>%
  pivot_longer(-Parameter, names_to = "names", values_to = "values") %>%
...
```{r}
tu_comp <- model_comp_interp_int(resp = "turnover_scaled")
...
```{r}
p_jt_sup_ne <- sum( abs(rigal_trends_df$turnover) >
  abs(rigal_trends_df$nestedness)) / nrow(rigal_trends_df)
```
```{r, include=FALSE}
