...
```{r}
library(spaMM)
tar_load(c(trend_env, slp_env))
names(trend_env) <- var_temporal_trends

summary(trend_env$jaccard_dis)

predict(trend_env$jaccard)[,1]
re <- paste0("(1 + dist_up_km + tmp_dc_cyr | ecoregion/main_bas/siteid)")
```

```{r}
re <- paste0("~ dist_up_km + (1 + dist_up_km | main_bas)")
...
```{r}
re <- paste0("~ tmp_dc_cyr + (1 + tmp_dc_cyr | main_bas)")
