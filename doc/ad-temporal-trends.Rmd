...
tar_load(c(rigal_trends, var_temporal_trends, rigal_slp_df))
...
cor_data <- modelling_data %>%
  left_join(hftc, by = "siteid") %>%
  select(
    !starts_with("cat") & !ends_with("scaled") & !starts_with("log_hft") & !starts_with("scaled")
    ) %>%
  select(-siteid, -main_bas, -unitabundance)
