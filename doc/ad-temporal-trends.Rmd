...
m2d <- spaMM::fitme(
  appearance ~ log_dist_up_km + (1 + log_dist_up_km | main_bas),
  stat_data
)
plot(m2d)
ti[[1]]
...
ti %>% reduce(., cbind)
ti %>%
  ggplot(aes(y = prediction, x = dist_up_km, color = main_bas)) +
  geom_point()

tm <- stat_data %>%
  na.omit() %>%
  mutate(
    pred_appearance = predict(m2d, re.form = ~log_dist_up_km + (1 + log_dist_up_km | main_bas))[,1]
  )
